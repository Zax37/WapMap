...
DWORD WINAPI _cDiscWatchdogThread(LPVOID phData)
{
    ...
    while(1){
     DWORD bytes = 0;
     if( ReadDirectoryChangesW(parent->hDirToWatch, &retbuffer, 2048, 1, FILE_NOTIFY_CHANGE_FILE_NAME|FILE_NOTIFY_CHANGE_LAST_WRITE, &bytes, 0, 0) ){
...
void cDiscFeed::_CreateWatchdog()
{
    ...
    hDirToWatch = CreateFile(strAbsoluteLocation.c_str(), FILE_LIST_DIRECTORY, FILE_SHARE_READ|FILE_SHARE_DELETE|FILE_SHARE_WRITE, 0, OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS, 0);
    DWORD dw;
...
unsigned char * cDiscFeed::GetFileContent(const char * path, unsigned int &oiDataLength, unsigned int iReadStart, unsigned int iReadLen)
{
    std::string npath = (bIsHDD ? std::string(path)
                                : strAbsoluteLocation + "/" + std::string(path));
    FILE * f = fopen(npath.c_str(), "rb");
...
std::vector<std::string> cDiscFeed::GetDirectoryContents(const char * path, bool bRecursive)
{
    ...
    while ((dirp = readdir(dp)) != NULL) {
     if( dirp->d_name[0] == '.' && (dirp->d_name[1] == '.' && dirp->d_name[2] == '\0' || dirp->d_name[1] == '\0') )
      continue;
     std::string entry = std::string(dirp->d_name);
...
{
    REZ::Element * el = hREZ->GetRoot()->GetElement(path);
    if( !el || !el->IsFile() ){ oiDataLength = 0; return 0; };
