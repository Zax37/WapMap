...
void cPhysicWorld::Update() {
    for (int i = 0; i < vhActiveBodies.size(); i++) {
        ...
        if (vhActiveBodies[i]->GetSpeedX() == 0 && vhActiveBodies[i]->GetSpeedY() == 0 &&
            !vhActiveBodies[i]->SleepBanned()) {
            ...
            if (hPl->GetTile(x, y)->GetID() != -1) {
                ...
                if (ta->GetType() == WWD::AtribType_Single) {
                    ...
                    if (ta->GetAtribInside() != WWD::Atrib_Clear) {
                        ...
                        new cPhysicBody(this, x * hPl->GetTileWidth() + ta->GetMask().x1,
                    ...
                    if (ta->GetAtribOutside() != WWD::Atrib_Clear) {
                        ...
                        if (ta->GetMask().y2 != 63)
                            new cPhysicBody(this, x * hPl->GetTileWidth(), y * hPl->GetTileHeight() + ta->GetMask().y2,
                                            64, 64 - ta->GetMask().y2, ta->GetAtribOutside());
                        if (ta->GetMask().x1 != 0)
                            new cPhysicBody(this, x * hPl->GetTileWidth(), y * hPl->GetTileHeight(), ta->GetMask().x1,
                                            ta->GetMask().y2, ta->GetAtribOutside());
                        if (ta->GetMask().x2 != 63)
                            new cPhysicBody(this, x * hPl->GetTileWidth() + ta->GetMask().x2, y * hPl->GetTileHeight(),
                                            64 - ta->GetMask().x2, ta->GetMask().y2, ta->GetAtribOutside());
                    }
                }
            }
        }
...
cPhysicBody::cPhysicBody(cPhysicWorld *w, float x1, float y1, float width, float height, WWD::Object *obj,
                         enBodyType it) {
    fX = x1;
    fY = y1;
    fW = width;
...
cPhysicBody::cPhysicBody(cPhysicWorld *w, float x1, float y1, float width, float height,
                         void(*pcolCB)(void *hUserData, cPhysicBody *trigger, cPhysicBody *body), void *phUD,
                         WWD::Object *obj) //trigger
{
    fX = x1;
    fY = y1;
    fW = width;
...
void cPhysicBody::Update() {
        ...
        if (iType == BODY_ELEVATOR) {
            for (int i = 0; i < vhCols.size(); i++) {
                if (vhCols[i]->GetType() == BODY_PLAYER) {
                    if (vhCols[i]->GetY() < fY) {
                        vhCols[i]->SetPos(vhCols[i]->GetX() + fSpeedX * hge->Timer_GetDelta(),
                                          vhCols[i]->GetY() + fSpeedY * hge->Timer_GetDelta());
                    }
                }
            }
        }
    ...
    if (hObj != NULL) {
        if (bGlueObject) {
            GetUserDataFromObj(hObj)->SetPos(fX + fW / 2 + iObjOffX * (GetUserDataFromObj(hObj)->GetFlipX() ? -1 : 1),
                                             fY + fH / 2 + iObjOffY);
        }
    }
}
