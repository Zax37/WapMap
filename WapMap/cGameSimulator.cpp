...
void cGameSimulator::Init()
{
    ...
    for(int p=0;p<hPar->GetPlanesCount();p++){
     ...
     for(int o=0;o<pl->GetObjectsCount();o++){
      ...
      if( !strcmp(obj->GetLogic(), "TProjectile") ){
       ...
       n.hObj = obj;
       n.fDelayTimer = 0;
       n.fDelayTime = obj->GetParam(WWD::Param_Speed) > 0 ? float(obj->GetParam(WWD::Param_Speed))/1000.0f : 500;
       ...
       int tileX = obj->GetParam(WWD::Param_LocationX)/tileW, tileY = obj->GetParam(WWD::Param_LocationY)/tileH;
       ...
       if( obj->GetParam(WWD::Param_SpeedX) == 0 )
        ...
        n.fFrameDuration = float(float(obj->GetParam(WWD::Param_SpeedX))/1000.0f) / float(iRopeFramesCount);
       if( obj->GetParam(WWD::Param_Speed) == 0 )
        n.iFrame = 0;
       else
        n.iFrame = int( (float(obj->GetParam(WWD::Param_Speed))/1000.0f) / n.fFrameDuration )%iRopeFramesCount;
       ...
       if( obj->GetHitRect().x1 != 0 || obj->GetHitRect().y1 != 0 || obj->GetHitRect().x2 != 0 || obj->GetHitRect().y2 != 0 ||
           ...
           obj->GetParam(WWD::Param_MaxX) != 0 || obj->GetParam(WWD::Param_MaxY) != 0 ) continue;
       cAmbientSound n;
       n.hObj = obj;
       n.fTimer = 0;
       n.fTime = 0.001;
       ...
       se->hPtr = (int*)n;
       GetUserDataFromObj(obj)->SetSpecialData((int*)se);
      }else if( !strcmp(obj->GetLogic(), "Soldier") || !strcmp(obj->GetLogic(), "Officer") || !strcmp(obj->GetLogic(), "Rat") ||
                !strcmp(obj->GetLogic(), "RobberThief") || !strcmp(obj->GetLogic(), "CutThroat") || !strcmp(obj->GetLogic(), "Seagull") ||
                ...
                !strcmp(obj->GetLogic(), "HermitCrab") || !strcmp(obj->GetLogic(), "PegLeg") || !strcmp(obj->GetLogic(), "CrazyHook") ||
                !strcmp(obj->GetLogic(), "Siren") || !strcmp(obj->GetLogic(), "Mercat") || !strcmp(obj->GetLogic(), "Fish") ||
       ...
       if( bAlignPos ){
        int atilex = n->hObj->GetParam(WWD::Param_LocationX)/GV->editState->GetActivePlane()->GetTileWidth(),
            atiley = n->hObj->GetParam(WWD::Param_LocationY)/GV->editState->GetActivePlane()->GetTileHeight();
        ...
        int newy = endtile*GV->editState->GetActivePlane()->GetTileHeight()-spr->GetHeight()/2+(hsy-spr->GetHeight()/2)+endoff;
        ...
        for(int x=atilex;x<pl->GetPlaneWidth();x++){
         ...
         if( GetTileColX(atr, 1) != 0 ){
          limitmax = (x+1)*GV->editState->GetActivePlane()->GetTileWidth()-spr->GetWidth()*0.25-GetTileColX(atr,1);
          break;
         }
        }
        for(int x=atilex;x>=0;x--){
         WWD::TileAtrib * atr = GV->editState->hParser->GetTileAtribs(pl->GetTile(x, endtile)->GetID());
         if( GetTileColY(atr) != endoff ){
          limitmin = (x+1)*GV->editState->GetActivePlane()->GetTileWidth()+spr->GetWidth()*0.25;
          break;
         }
         if( GetTileColX(atr, 0) != 0 ){
          limitmin = x*GV->editState->GetActivePlane()->GetTileWidth()+spr->GetWidth()*0.25+GetTileColX(atr,0);
          break;
         }
        }
    ...
    GV->Console->Printf("~w~Found %d/%d active objects.",
                        ...
                        vSndTrig.size()   + vPElevators.size() + vAmbients.size()   + vProjSpawner.size() + vCrumbPeg.size() +
...
void cGameSimulator::Think()
{
    ...
    if( GV->editState->vPort->GetWidget()->isMouseOver() ){
     ...
     std::vector<WWD::Object*> objv = GV->editState->hPlaneData[GV->editState->GetActivePlaneID()]->ObjectData.hQuadTree->GetObjectsByArea(wx, wy, 2, 2);
     ...
     if( obj != NULL ){
      ...
      if( hge->Input_KeyDown(HGEK_LBUTTON) ){
       ...
       if( type == SE_Elevator ){
        cElevator * el = (cElevator*)(((stObjUserDataSimEntity*)GetUserDataFromObj(obj)->GetSpecialData())->hPtr);
        ...
        cCrumblingPeg * peg = (cCrumblingPeg*)(((stObjUserDataSimEntity*)GetUserDataFromObj(obj)->GetSpecialData())->hPtr);
        peg->bRun = 1;
        ...
        cPowderKeg * keg = (cPowderKeg*)(((stObjUserDataSimEntity*)GetUserDataFromObj(obj)->GetSpecialData())->hPtr);
        keg->bExplode = 1;
        ...
        cCheckpoint * cp = (cCheckpoint*)(((stObjUserDataSimEntity*)GetUserDataFromObj(obj)->GetSpecialData())->hPtr);
        cp->fTimer = 0;
        PlayAnimSound(aniFlagRise->GetFrame(0));
        delete ((stObjUserDataSimEntity*)GetUserDataFromObj(obj)->GetSpecialData());
        GetUserDataFromObj(obj)->SetSpecialData(0);
       }else if( type == SE_Enemy ){
        cEnemy * n = (cEnemy*)(((stObjUserDataSimEntity*)GetUserDataFromObj(obj)->GetSpecialData())->hPtr);
        n->bHit = 1;
        n->iHitFrame = 0;
        ...
        GV->editState->fCamX = (n->hObj->GetParam(WWD::Param_SpeedX)-GV->editState->vPort->GetWidth()/2)*GV->editState->fZoom;
        GV->editState->fCamY = (n->hObj->GetParam(WWD::Param_SpeedY)-GV->editState->vPort->GetHeight()/2)*GV->editState->fZoom;
    ...
    for(int i=0;i<vTogglePegs.size();i++){
     ...
     if( vTogglePegs[i].bOn ){
      ...
      }else if( vTogglePegs[i].fCounter > vTogglePegs[i].fTimeOff-vTogglePegs[i].iFramesCount*vTogglePegs[i].fFrameDuration ){
       vTogglePegs[i].iFrame = vTogglePegs[i].iFramesCount - (vTogglePegs[i].fCounter - (vTogglePegs[i].fTimeOff-vTogglePegs[i].iFramesCount*vTogglePegs[i].fFrameDuration)) / vTogglePegs[i].fFrameDuration;
    ...
    int scrx1 = GV->editState->Scr2WrdX(GV->editState->GetActivePlane(), GV->editState->vPort->GetX()),
        ...
        scrx2 = GV->editState->Scr2WrdX(GV->editState->GetActivePlane(), GV->editState->vPort->GetX()+GV->editState->vPort->GetWidth()),
        scry2 = GV->editState->Scr2WrdY(GV->editState->GetActivePlane(), GV->editState->vPort->GetY()+GV->editState->vPort->GetHeight());
    for(int i=0;i<vCheckpoint.size();i++){
     ...
     if( fr != vCheckpoint[i]->iFrame )
      GetUserDataFromObj(vCheckpoint[i]->hObj)->SetI(GV->editState->SprBank->GetAssetByID(vCheckpoint[i]->hObj->GetImageSet())->GetIMGByID(ani->GetFrame(vCheckpoint[i]->iFrame)->GetImageID())->GetIt()+1);
    }
    for(int i=0;i<vPowderKeg.size();i++){
     if( vPowderKeg[i]->bExplode ){
      ...
      if( vPowderKeg[i]->fAnimTimer*1000.0f > aniPowderExpl->GetFrame(ud->GetI())->GetDuration() ){
       ...
       newi++;
       if( newi == 2 ){
        std::vector<WWD::Object*> objv = GV->editState->hPlaneData[GV->editState->GetActivePlaneID()]->ObjectData.hQuadTree->GetObjectsByArea(vPowderKeg[i]->hObj->GetParam(WWD::Param_LocationX)-75,
                                                                                                                                              vPowderKeg[i]->hObj->GetParam(WWD::Param_LocationY)-75,
                                                                                                                                              150, 150);
        for(int z=0;z<objv.size();z++){
         if( GetUserDataFromObj(objv[z])->GetSpecialData() != 0 &&
             ((stObjUserDataSimEntity*)GetUserDataFromObj(objv[z])->GetSpecialData())->enType == SE_PowderKeg ){
          cPowderKeg * keg = (cPowderKeg*)(((stObjUserDataSimEntity*)GetUserDataFromObj(objv[z])->GetSpecialData())->hPtr);
          keg->bExplode = 1;
          hge->Effect_PlayEx(GV->editState->hSndBank->GetAssetByID("GAME_KEGBOOM")->GetSound(), 100);
    ...
    for(int i=0;i<vCrumbPeg.size();i++){
     if( vCrumbPeg[i]->bRun ){
      ...
      if( vCrumbPeg[i]->fAnimTimer > 0.05f ){
       ...
       int newi = ud->GetI();
       newi++;
       if( newi >= GV->editState->SprBank->GetAssetByID(vCrumbPeg[i]->hObj->GetImageSet())->GetSpritesCount() ){
        ud->SetVisible(0);
        delete vCrumbPeg[i];
      ...
      if( vProjSpawner[i].vhChildren[p]->bPhase ){ //flying
       ...
       if( GV->editState->hParser->GetBaseLevel() == 14 ){
        ...
        if( vProjSpawner[i].vhChildren[p]->fAnimTimer > 0.075 ){
         ...
         int newi = ud->GetI();
         newi++;
         if( newi >= GV->editState->SprBank->GetAssetByID(vProjSpawner[i].vhChildren[p]->hObj->GetImageSet())->GetSpritesCount() )
          newi = 0;
         ud->SetI(newi);
        }
       }
       if( abs(ud->GetX()-vProjSpawner[i].hObj->GetParam(WWD::Param_LocationX)) > vProjSpawner[i].iDistance ||
           abs(ud->GetY()-vProjSpawner[i].hObj->GetParam(WWD::Param_LocationY)) > vProjSpawner[i].iDistance ){
         GV->editState->GetActivePlane()->DeleteObject(vProjSpawner[i].vhChildren[p]->hObj);
         vProjSpawner[i].vhChildren.erase(vProjSpawner[i].vhChildren.begin()+p);
         p--;
       }
       ...
       if( vProjSpawner[i].vhChildren[p]->fAnimTimer*1000 > ani->GetFrame(ud->GetI())->GetDuration() ){
        ...
        if( ani->GetFramesCount() > ud->GetI()+1 ){
         ...
         if( GV->editState->hParser->GetBaseLevel() == 14 ){
          ...
          vProjSpawner[i].vhChildren[p]->hObj->SetImageSet(vProjSpawner[i].fSpeedX < 0 ? "LEVEL_PROJECTILES_1" : "LEVEL_PROJECTILES_2");
         }
        }
       }
      }
    ...
    for(int i=0;i<vEyeCandies.size();i++){
     ...
     while( vEyeCandies[i].fCounter > SIM_EYECANDY_DELAY ){
      ...
      if( vEyeCandies[i].iFrame >= mO->SprBank->GetAssetByID(vEyeCandies[i].hObj->GetImageSet())->GetSpritesCount() )
       vEyeCandies[i].iFrame -= mO->SprBank->GetAssetByID(vEyeCandies[i].hObj->GetImageSet())->GetSpritesCount();
    ...
    for(int i=0;i<vAmbients.size();i++){
     ...
     if( !vAmbients[i].bPlaying && vAmbients[i].bOn ){
      ...
      if( as != NULL )
       vAmbients[i].hChan = hge->Effect_PlayEx(as->GetSound(), (vAmbients[i].hObj->GetParam(WWD::Param_Damage) != 0 ? vAmbients[i].hObj->GetParam(WWD::Param_Damage) : 100));
     ...
     if( el->bMoving ){
      ...
      if( bBorder ){
       ...
       if( idle == 0 || !el->bStopping ){
        ...
        el->iAniFrame = 0;
        char anim[256];
        sprintf(anim, "%s_%s", el->hObj->GetImageSet(), el->vIdleAnims[hge->Random_Int(0, el->vIdleAnims.size()-1)].c_str());
    ...
    for(int i=0;i<vPElevators.size();i++){
     ...
     if( n->vPaths[n->iStep].first == 5 && n->fSleep >= n->vPaths[n->iStep].second ||
        abs(n->fPosX-n->fOrigX) > n->vPaths[n->iStep].second || abs(n->fPosY-n->fOrigY) > n->vPaths[n->iStep].second ){
      ...
      if( n->vPaths[n->iStep].first == 5 ){
       ...
       if(      n->vPaths[n->iStep].first == 7 || n->vPaths[n->iStep].first == 4 || n->vPaths[n->iStep].first == 1 ) n->iDirHor = -1;
       else if( n->vPaths[n->iStep].first == 8 ||                                   n->vPaths[n->iStep].first == 2 ) n->iDirHor = 0;
       else if( n->vPaths[n->iStep].first == 9 || n->vPaths[n->iStep].first == 6 || n->vPaths[n->iStep].first == 3 ) n->iDirHor = 1;
      }
      n->fSleep = 0;
