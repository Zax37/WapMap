     ...
     for(int y = 0; y < m_vhSets[x]->GetTilesCount(); y++){
      ...
      while( vTexes[texidx]->GetFreeSlotsNum() == 0 ){
       if( texidx == vTexes.size()-1 ){
        ...
        cTilesetTexture * ntex = new cTilesetTexture(txw/64, txh/64);
        vTexes.push_back(ntex);
        GV->Console->Printf("~w~Registered %dx%d tile texture to store %d tiles.", txw, txh, toloadc-caret);
       }
       texidx++;
      }
...
void cBankTile::ReloadBrushes()
{
    ...
    while( (pent = readdir(dir)) != NULL ){
     if( pent == NULL ){
      GV->Console->Printf("~r~Error iterating brushes (DIRENT FAIL)!~w~");
      return;
     }
     struct stat StatBuffer;
     char temp[256];
     sprintf(temp, "brush/%s", pent->d_name);
     stat(temp, &StatBuffer);
     if( !(StatBuffer.st_mode & S_IFDIR) ){
      char * ext = SHR::GetExtension(pent->d_name);
      char * lext = SHR::ToLower(ext);
      delete [] ext;
      if( pent->d_name[0] == sign && !strcmp("lua", lext) || !strcmp("wbp", lext) ){
       cBrush * tmp = NULL;
       if( !strcmp("lua", lext) )
        while(1){
         tmp = new cBrush(pent->d_name, m_hParser, vszLayers);
         if( tmp->GetStatus() == BrushLuaError || tmp->GetStatus() == BrushOtherError ){
          char caption[256];
          sprintf(caption, "%s: %s", GETL(Lang_BrushErrorCaption), pent->d_name);
          char desc[256];
          ...
          delete tmp;
          int r = MessageBox(hge->System_GetState(HGE_HWND), desc, caption, MB_RETRYCANCEL | MB_ICONERROR | MB_DEFBUTTON2 | MB_APPLMODAL);
          if( r == 5 ){ //continue
           GV->Console->Printf("~r~Error loading brush; skipping~w~ (~y~%s~w~)", pent->d_name);
           break;
          }else if( r == 4 ){ //retry
           GV->Console->Printf("~r~Error loading brush; retrying~w~ (~y~%s~w~)", pent->d_name);
           continue;
          }
          ...
          GV->Console->Printf("~r~Rejected brush ~y~%s~r~.", pent->d_name);
          delete tmp;
          break;
         }
        }
        ...
        char tmpch[128];
        sprintf(tmpch, "brush/%s", pent->d_name);
        ...
        GV->Console->Printf("Parsing ~y~%s ~w~package...", pent->d_name);
        ...
        GV->Console->Printf("End of ~y~%s ~w~package.", pent->d_name);
...
cTileImageSet * cBankTile::GetSet(const char * pszSet, bool bCaseSensitive)
{
    ...
    for(int i=0;i<m_vhSets.size();i++){
     if(  bCaseSensitive && !strcmp(pszSet, m_vhSets[i]->m_szName) ||
         !bCaseSensitive && !stricmp(pszSet, m_vhSets[i]->m_szName) ){
      return m_vhSets[i];
     }
    }
    return NULL;
    ...
    for(size_t i=0;i<mountEntry.vFiles.size();i++){
     if( !hDC->IsLoadableImage(mountEntry.vFiles[i], &imginf) ){
      ...
      if( mountEntry.vFiles[i].hFeed == hDC->GetFeed(DB_FEED_REZ) )
       ...
       src = "CUS";
      GV->Console->Printf("~r~'~y~[%s]/%s~r~' is not a supported image file.", src, mountEntry.vFiles[i].strPath.c_str());
      continue;
     }else{
      bfnd = 1;
