...
namespace ObjEdit {
    cEditObjCrate::cEditObjCrate(WWD::Object *obj, State::EditingWW *st) : cObjEdit(obj, st) {
        ...
        rbType[0] = new SHR::RadBut(GV->hGfxInterface, GETL2S("EditObj_Statue", "Behind"), tmp,
                                    strstr(hTempObj->GetLogic(), "Behind") || strstr(hTempObj->GetLogic(), "Back"));
        rbType[0]->adjustSize();
    ...
    void cEditObjCrate::Draw() {
        ...
        hge->Gfx_RenderLine(dx, dy + win->getHeight() - 125, dx + win->getWidth(), dy + win->getHeight() - 125,
                            GV->colLineDark);
        hge->Gfx_RenderLine(dx, dy + win->getHeight() - 124, dx + win->getWidth(), dy + win->getHeight() - 124,
                            GV->colLineBright);
        ...
        if (bStackable) {
            for (int i = 8; i >= 0; i--)
                if (invTabs[i]->isVisible()) {
                    hState->SprBank->GetObjectSprite(hTempObj)->Render(dx + 117,
                                                                       dy + 150 + i * 43 + 75 * bShowWarpOptions);
                }
        }
    }
