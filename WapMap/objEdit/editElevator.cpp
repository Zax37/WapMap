...
namespace ObjEdit {
    ...
    cEditObjElevator::cEditObjElevator(WWD::Object *obj, State::EditingWW *st) : cObjEdit(obj, st) {
        ...
        winProp->setShadow(0);
        st->conMain->add(winProp, st->vPort->GetX() + winType->getWidth(),
                         st->vPort->GetY() + st->vPort->GetHeight() - winProp->getHeight());
    ...
    void cEditObjElevator::RebuildWindow() {
        ...
        if (!tfSpeedX->isEnabled())
            ...
            char tmp[64];
            sprintf(tmp, "%d",
                    hTempObj->GetParam(WWD::Param_SpeedX) == 0 ? 125 : hTempObj->GetParam(WWD::Param_SpeedX));
            tfSpeedX->setText(tmp);
        }
        ...
        if (!tfSpeedY->isEnabled())
            ...
            char tmp[64];
            sprintf(tmp, "%d",
                    hTempObj->GetParam(WWD::Param_SpeedY) == 0 ? 125 : hTempObj->GetParam(WWD::Param_SpeedY));
            tfSpeedY->setText(tmp);
        }
