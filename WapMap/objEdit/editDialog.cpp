...
namespace ObjEdit
{
	cEditObjDialog::cEditObjDialog(WWD::Object * obj, State::EditingWW * st) : cObjEdit(obj, st)
	{
		...
		hPlayer = new cSoundPlayer(win, 5, 78 + 25,
			hState->hSndBank->GetAssetByID(hTempObj->GetAnim()), GETL2S("EditObj_Dialog", "Preview"));
	}
	...
	void cEditObjDialog::Action(const gcn::ActionEvent &actionEvent)
	{
		if (actionEvent.getSource() == win) {
			...
			hPlayer->SetAsset(hState->hSndBank->GetAssetByID(vsStandardSounds[ddStandardDialogs->getSelected()].first.c_str()), 0);
		}
