...
namespace ObjEdit
{
 ...
 void cInvPickbox::Think()
 {
    ...
    int pickedid = -1;
    if( GV->editState->conMain->getWidgetAt(mx,my) == win && mx > dx+4 && my > dy+28 && mx < dx+4+252 && my < dy+28+267 ){
     ...
     if( restx > 5 && restx < 53 && resty > 5 && resty < 53 ){
      ...
      if( pickedid < InventoryItemsCount ){
       if( hge->Input_KeyDown(HGEK_LBUTTON) )
        GV->editState->hInvCtrl->SetItemInClipboard(GV->editState->hInvCtrl->GetItemByIt(pickedid), restx-30, resty-30);
 ...
 void cInvPickbox::Draw(int iCode)
 {
    ...
    for(int i=1;i<6;i++){
     hge->Gfx_RenderLine(dx+4, dy+29+i*60-2-lineoff, dx+4+238, dy+29+i*60-2-lineoff, GV->colLineDark);
     hge->Gfx_RenderLine(dx+4, dy+29+i*60-1-lineoff, dx+4+238, dy+29+i*60-2-lineoff, GV->colLineBright);
    }
    for(int i=1;i<4;i++)
    ...
    for(int y=startrow;y<endrow;y++)
     for(int x=0;x<4;x++){
      ...
      if( id < InventoryItemsCount ){
       cSprBankAsset * asset = GV->editState->SprBank->GetAssetByID(GV->editState->hInvCtrl->GetItemByIt(id).first.c_str());
    ...
    float mx,my;
    hge->Input_GetMousePos(&mx, &my);
    if( GV->editState->conMain->getWidgetAt(mx,my) == win && mx > dx+4 && mx > dy+28 && mx < dx+4+252 && my < dy+28+267 ){
     mx -= dx+4;
     my -= dy+28;
     my += off;
     int restx = int(mx)%60, resty = int(my)%60;
