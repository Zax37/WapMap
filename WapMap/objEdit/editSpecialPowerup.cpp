...
namespace ObjEdit
{
	...
	cEditObjSpecialPowerup::cEditObjSpecialPowerup(WWD::Object * obj, State::EditingWW * st) : cObjEdit(obj, st)
	{
		...
		for (int i = 0; i < 8; i++) {
			...
			rbType[i] = new SHR::RadBut(GV->hGfxInterface, "", tmp,
				!strcmp(hTempObj->GetImageSet(), hState->hInvCtrl->GetItemByID(iSpecialPowerupID[i]).first.c_str()));
			rbType[i]->adjustSize();
	...
	void cEditObjSpecialPowerup::Draw()
	{
		...
		for (int i = 1; i < 4; i++)
			hge->Gfx_RenderLine(dx + 5 + i * 71 - 1, dy + 28, dx + 5 + i * 71 - 1, dy + 29 + 267 - 150, GV->colLineBright);
		for (int i = 1; i < 3; i++) {
		...
		for (int i = 1; i < 4; i++)
			hge->Gfx_RenderLine(dx + 5 + i * 71 - 2, dy + 28, dx + 5 + i * 71 - 2, dy + 29 + 267 - 150, GV->colLineDark);
		...
		for (int i = 0; i < 8; i++) {
			int x = i % 4, y = i / 4;
			cSprBankAsset * asset = hState->SprBank->GetAssetByID(hState->hInvCtrl->GetItemByID(iSpecialPowerupID[i]).first.c_str());
