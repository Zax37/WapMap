...
namespace ObjEdit
{
 ...
 cEditObjCurse::cEditObjCurse(WWD::Object * obj, State::EditingWW * st) : cObjEdit(obj, st)
 {
    ...
    for(int i=0;i<6;i++){
     ...
     rbCurses[i] = new SHR::RadBut(GV->hGfxInterface, GETL2S("EditObj_Curse", tmp2), tmp, !(strcmp(obj->GetImageSet(), CURSES[i])));
 ...
 void cEditObjCurse::Draw()
 {
    ...
    for(int i=0;i<6;i++){ //131
     ...
     hge->Gfx_RenderLine(dx+i*131, dy+24, dx+i*131, i < 5 ? dy+120 : dy+win->getHeight(), 0xFF1f1f1f);
     hge->Gfx_RenderLine(dx+i*131+1, dy+24, dx+i*131+1, i < 5 ? dy+120 : dy+win->getHeight(), 0xFF5c5c5c);
    }
    ...
    GV->fntMyriad13->printf(dx+15, dy+130, HGETEXT_LEFT,
                            "%s: ~y~%s~l~", 0, GETL2S("EditObj_Curse", "CurseEffect"), GETL2S("EditObj_Curse", tmp));
 }
}
