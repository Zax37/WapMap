...
namespace ObjEdit
{
	cEditObjTogglePeg::cEditObjTogglePeg(WWD::Object * obj, State::EditingWW * st) : cObjEdit(obj, st)
	{
		...
		for (int i = 0; i < vszDefaultImgsets.size(); i++) {
			rbType[i] = new SHR::RadBut(GV->hGfxInterface, "", tmp, !strcmp(hTempObj->GetImageSet(), vszDefaultImgsets[i].c_str()));
			rbType[i]->adjustSize();
		...
		st->conMain->add(winSeries, st->vPort->GetX() + win->getWidth(), st->vPort->GetY() + st->vPort->GetHeight() - winSeries->getHeight());
	}
	...
	void cEditObjTogglePeg::Save()
	{
		...
		if (delay == 750) { delay = 0; hTempObj->SetLogic("TogglePeg2"); }
		else if (delay == 750 * 2) { delay = 0; hTempObj->SetLogic("TogglePeg3"); }
		else if (delay == 750 * 3) { delay = 0; hTempObj->SetLogic("TogglePeg4"); }
	...
	void cEditObjTogglePeg::Draw()
	{
		...
		for (int i = 0; i < 3; i++) {
			if (rbType[i] != 0) {
				hgeSprite * spr = hState->SprBank->GetAssetByID(vszDefaultImgsets[i].c_str())->GetIMGByIterator(0)->GetSprite();
				...
				if (mx > dx + 37 + i * 73 + 13 + float(r.x1)*fscale && mx < dx + 37 + i * 73 + 13 + float(r.x2)*fscale &&
