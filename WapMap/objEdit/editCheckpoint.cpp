...
namespace ObjEdit
{
 cEditObjCheckpoint::cEditObjCheckpoint(WWD::Object * obj, State::EditingWW * st) : cObjEdit(obj, st)
 {
    ...
    rbType[0] = new SHR::RadBut(GV->hGfxInterface, GETL2S("EditObj_Checkpoint", "Checkpoint"), tmp, !strcmp(hTempObj->GetImageSet(), "GAME_CHECKPOINTFLAG"));
    rbType[0]->adjustSize();
    rbType[0]->addActionListener(hAL);
    win->add(rbType[0], 5, 10);
    rbType[1] = new SHR::RadBut(GV->hGfxInterface, GETL2S("EditObj_Checkpoint", "SuperCheckpoint"), tmp, !strcmp(hTempObj->GetImageSet(), "GAME_SUPERCHECKPOINT"));
    rbType[1]->adjustSize();
 ...
 void cEditObjCheckpoint::Draw()
 {
    ...
    hState->SprBank->GetAssetByID("GAME_CHECKPOINTFLAG")->GetIMGByID(iCpAnimFrame)->GetSprite()->Render(dx+124, dy+100);
    hState->SprBank->GetAssetByID("GAME_SUPERCHECKPOINT")->GetIMGByID(iScpAnimFrame)->GetSprite()->Render(dx+124, dy+250);
