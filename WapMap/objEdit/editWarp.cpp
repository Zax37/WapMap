...
namespace ObjEdit {
    cEditObjWarp::cEditObjWarp(WWD::Object *obj, State::EditingWW *st) : cObjEdit(obj, st) {
        ...
        rbType[0] = new SHR::RadBut(GV->hGfxInterface, GETL2S("EditObj_Warp", "Hor"), tmp,
                                    !strcmp(hTempObj->GetImageSet(), "GAME_WARP"));
        rbType[0]->adjustSize();
        rbType[0]->addActionListener(hAL);
        win->add(rbType[0], 5, 10);
        rbType[1] = new SHR::RadBut(GV->hGfxInterface, GETL2S("EditObj_Warp", "Ver"), tmp,
                                    !strcmp(hTempObj->GetImageSet(), "GAME_VERTWARP"));
        rbType[1]->adjustSize();
        rbType[1]->addActionListener(hAL);
        win->add(rbType[1], 5, 80);
        rbType[2] = new SHR::RadBut(GV->hGfxInterface, GETL2S("EditObj_Warp", "Boss"), tmp,
                                    !strcmp(hTempObj->GetImageSet(), "GAME_BOSSWARP"));
        rbType[2]->adjustSize();
    ...
    void cEditObjWarp::Draw() {
        ...
        GV->fntMyriad16->Render(dx + 5, dy + 342 - 25, HGETEXT_LEFT, "Y:", 0);
        ...
        hge->Gfx_RenderLine(dx, dy + 295 + 67 - (rbType[2]->isSelected() * 22), dx + win->getWidth(),
                            dy + 295 + 67 - (rbType[2]->isSelected() * 22), GV->colLineDark);
        hge->Gfx_RenderLine(dx, dy + 295 + 68 - (rbType[2]->isSelected() * 22), dx + win->getWidth(),
                            dy + 295 + 68 - (rbType[2]->isSelected() * 22), GV->colLineBright);
