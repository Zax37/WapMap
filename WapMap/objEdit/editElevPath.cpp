...
namespace ObjEdit
{
 ...
 std::string lmEditObjElevPath::getElementAt(int i)
 {
    ...
    if( m_hOwn->vSteps[i].first == 5 ){
     ...
     sprintf(tmp, "%s (X: %d, Y: %d)", GETL2S("EditObj_ElevPath", "Move"), m_hOwn->GetMoveX(i), m_hOwn->GetMoveY(i));
    }
    return tmp;
 }
 ...
 {
    delete butInvX; delete butInvY;
    delete tddImageSet;
    delete butInvertPath;
    delete labMove; delete labWaitTime; delete labType; delete labSteps; delete labSpeed; delete labImgSet; delete labDir;
    delete rbMove; delete rbWait;
    for(int i=0;i<8;i++) delete butMoveDir[i];
    delete tfStepMove; delete tfStepWaitTime; delete tfSpeed;
    delete lbSteps;
    delete lmDefImgSets;
    delete lmSteps;
    delete butStepAdd; delete butStepDel;
    delete butGenChain; delete butCloseChain;
    delete win;
 ...
 void cEditObjElevPath::Save()
 {
     ...
     if( (endX != initX || endY != initY) &&
         MessageBox(hge->System_GetState(HGE_HWND), GETL2S("EditObj_ElevPath", "MsgDifferentEnd"), GETL(Lang_Message), MB_YESNO|MB_ICONWARNING) == IDNO )
      return;
 ...
 void cEditObjElevPath::Action(const gcn::ActionEvent &actionEvent)
 {
    if( actionEvent.getSource() == win ){
     ...
     if( ChangesMade() )
      if( MessageBox(hge->System_GetState(HGE_HWND), GETL2S("EditObj_ElevPath", "MsgGenChainChangesMade"), GETL(Lang_Message), MB_YESNO|MB_ICONINFORMATION) == IDNO )
       return;
 ...
 void cEditObjElevPath::Draw()
 {
    ...
    hge->Gfx_RenderLine(dx+lbSteps->getX(), dy+lbSteps->getY()+15, dx+lbSteps->getX()+lbSteps->getWidth()+3, dy+lbSteps->getY()+15, 0xFF1f1f1f);
    hge->Gfx_RenderLine(dx+lbSteps->getX(), dy+lbSteps->getY()+16, dx+lbSteps->getX()+lbSteps->getWidth()+4, dy+lbSteps->getY()+16, 0xFF5c5c5c);
    hge->Gfx_RenderLine(dx+lbSteps->getX(), dy+lbSteps->getY()+152+17, dx+lbSteps->getX()+lbSteps->getWidth()+3, dy+lbSteps->getY()+152+17, 0xFF1f1f1f);
    hge->Gfx_RenderLine(dx+lbSteps->getX(), dy+lbSteps->getY()+152+18, dx+lbSteps->getX()+lbSteps->getWidth()+4, dy+lbSteps->getY()+152+18, 0xFF5c5c5c);

    hge->Gfx_RenderLine(dx+lbSteps->getX()+1, dy+lbSteps->getY()+15, dx+lbSteps->getX()+1, dy+lbSteps->getY()+152+16, 0xFF1f1f1f);
    hge->Gfx_RenderLine(dx+lbSteps->getX()+2, dy+lbSteps->getY()+16, dx+lbSteps->getX()+2, dy+lbSteps->getY()+152+16, 0xFF5c5c5c);
    hge->Gfx_RenderLine(dx+lbSteps->getX()+lbSteps->getWidth()+3, dy+lbSteps->getY()+15, dx+lbSteps->getX()+lbSteps->getWidth()+3, dy+lbSteps->getY()+152+16, 0xFF1f1f1f);
    hge->Gfx_RenderLine(dx+lbSteps->getX()+lbSteps->getWidth()+4, dy+lbSteps->getY()+15, dx+lbSteps->getX()+lbSteps->getWidth()+4, dy+lbSteps->getY()+152+16, 0xFF5c5c5c);
