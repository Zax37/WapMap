...
class cPhysicBody {
 private:
  ...
  bool bNoSleep;
 public:
  cPhysicBody(cPhysicWorld * w, float x1, float y1, float width, float height, WWD::TILE_ATRIB atr = WWD::Atrib_Clear); //static
  cPhysicBody(cPhysicWorld * w, float x1, float y1, float width, float height, WWD::Object * obj, enBodyType it); //dynamic
  cPhysicBody(cPhysicWorld * w, float x1, float y1, float width, float height, void (*pcolCB)(void * hUserData, cPhysicBody * trigger, cPhysicBody * body), void * phUD, WWD::Object * obj = NULL); //trigger
  ~cPhysicBody();
  void SetX(float x);
  void SetY(float y);
  ...
  cPhysicBody * GetCollisionBody(int i){ if( i >= 0 && i < vhCols.size() ) return vhCols[i]; return NULL; };
  ...
  void SetObjectOffset(int x, int y){ iObjOffX = x; iObjOffY = y; };

  void SetSpeed(float x, float y){ fSpeedX = x; fSpeedY = y; if( fSpeedX != 0 || fSpeedY != 0 ) WakeUp(); };
  void SetSpeedX(float x){ fSpeedX = x; if( fSpeedX != 0 ) WakeUp(); };
  void SetSpeedY(float y){ fSpeedY = y; if( fSpeedY != 0 ) WakeUp(); };
