...
winImageSetBrowser::winImageSetBrowser()
{
	...
	saImageSets->setDimension(gcn::Rectangle(0, 0, CONST_IMGSETBROWSER_ISLISTW, 530));
	saImageSets->setOpaque(0);
...
void winImageSetBrowser::Think()
{
	...
	int tilePickX = dx + CONST_IMGSETBROWSER_ISLISTW + 15, tilePickW = myWin->getWidth() - (CONST_IMGSETBROWSER_ISLISTW + 15) - 16,
	...
	if (mx > tilePickX && my > tilePickY && mx < tilePickX + tilePickW && my < tilePickY + tilePickH) {
		if (bSingleGroup) {
			iHighlightedF = MouseHandleGroup(*hSingleGroup, tilePickX + borderoffset, tilePickY + 10 - scroll, tilesPerRow);
		}
		else {
			iHighlightedF = -1;
			int ypos = 0;
	...
	GV->editState->bShowHand = (iHighlightedIS != -1 && iHighlightedIS != iSelectedImageSet
		|| iHighlightedF != -1 && iHighlightedF != iSelectedFrame);
}
...
void winImageSetBrowser::Draw(int piCode)
{
	...
	hge->Gfx_RenderLine(dx + CONST_IMGSETBROWSER_ISLISTW + 12, dy + 25, dx + CONST_IMGSETBROWSER_ISLISTW + 12, dy + myWin->getHeight(), GV->colLineDark);
	hge->Gfx_RenderLine(dx + CONST_IMGSETBROWSER_ISLISTW + 13, dy + 25, dx + CONST_IMGSETBROWSER_ISLISTW + 13, dy + myWin->getHeight(), GV->colLineBright);

	//tile set properties separator
	hge->Gfx_RenderLine(dx + CONST_IMGSETBROWSER_ISLISTW + 11, dy + 100, dx + myWin->getWidth(), dy + 100, GV->colLineDark);
	hge->Gfx_RenderLine(dx + CONST_IMGSETBROWSER_ISLISTW + 11, dy + 100, dx + myWin->getWidth(), dy + 101, GV->colLineBright);

	//tiles separator
	hge->Gfx_RenderLine(dx + CONST_IMGSETBROWSER_ISLISTW + 11, dy + 280, dx + myWin->getWidth(), dy + 280, GV->colLineDark);
	hge->Gfx_RenderLine(dx + CONST_IMGSETBROWSER_ISLISTW + 11, dy + 281, dx + myWin->getWidth(), dy + 281, GV->colLineBright);


	hge->Gfx_SetClipping(dx + 6, dy + 45, CONST_IMGSETBROWSER_ISLISTW - 9, 530);
	for (int y = 0; y <= 530 / 128; y++)
		for (int x = 0; x <= (CONST_IMGSETBROWSER_ISLISTW - 9) / 128; x++)
	...
	if (bank->GetAssetsCount() > 0) {
		...
		for (int i = startSet; i < bank->GetAssetsCount() && i < startSet + (saImageSets->getHeight() / CONST_IMGSETBROWSER_LISTSETH) + 2; i++) {
			int drawX = dx + 8;
			...
			if (i == iSelectedImageSet || i == iHighlightedIS) {
				...
				q.tex = 0;
				SHR::SetQuad(&q, (i == iSelectedImageSet ? 0x331796ff : 0x33a1a1a1), drawX, drawY, drawX + (CONST_IMGSETBROWSER_ISLISTW - 13), drawY + CONST_IMGSETBROWSER_LISTSETH);
				hge->Gfx_RenderQuad(&q);
			}

			int iPreviewDim = CONST_IMGSETBROWSER_LISTSETH - 20;

			hge->Gfx_RenderLine(drawX + 10 - 2, drawY + 10 - 1, drawX + 10 + iPreviewDim + 2, drawY + 10 - 1, GV->colLineDark);
			hge->Gfx_RenderLine(drawX + 10 - 1, drawY + 10 + iPreviewDim + 2, drawX + 10 + iPreviewDim + 2, drawY + 10 + iPreviewDim + 2, GV->colLineDark);
			hge->Gfx_RenderLine(drawX + 10 - 1, drawY + 10 - 1, drawX + 10 - 1, drawY + 10 + iPreviewDim + 2, GV->colLineDark);
			hge->Gfx_RenderLine(drawX + 10 + iPreviewDim + 2, drawY + 10 - 1, drawX + 10 + iPreviewDim + 2, drawY + 10 + iPreviewDim + 2, GV->colLineDark);

			hge->Gfx_RenderLine(drawX + 10 - 1, drawY + 10, drawX + 10 + iPreviewDim + 1, drawY + 10, GV->colLineBright);
			hge->Gfx_RenderLine(drawX + 10, drawY + 10 + iPreviewDim + 1, drawX + 10 + iPreviewDim + 1, drawY + 10 + iPreviewDim + 1, GV->colLineBright);
			hge->Gfx_RenderLine(drawX + 10, drawY + 10, drawX + 10, drawY + 10 + iPreviewDim + 1, GV->colLineBright);
			hge->Gfx_RenderLine(drawX + 10 + iPreviewDim + 1, drawY + 10, drawX + 10 + iPreviewDim + 1, drawY + 10 + iPreviewDim + 1, GV->colLineBright);
			...
			bool first = 1;
			while (GV->fntMyriad13->GetStringWidth(str.c_str(), 0) > strw) {
...
void winImageSetBrowser::Synchronize()
{
	...
	char * buf = SHR::FormatTimeFromUnix(pickedtile->GetFile().hFeed->GetFileModTime(pickedtile->GetFile().strPath.c_str()));
...
void winImageSetBrowser::action(const ActionEvent& actionEvent)
{
	if (actionEvent.getSource() == butBrowseFolder) {
		cFile f = GV->editState->SprBank->GetAssetByIterator(iSelectedImageSet)->GetIMGByIterator(iSelectedFrame)->GetFile();
		...
		GV->StateMgr->Push(new State::ImageImport(State::ImageImportObject, GV->editState->SprBank->GetAssetByIterator(iSelectedImageSet)));
	}
		...
		cSprBankAssetIMG * t = GV->editState->SprBank->GetAssetByIterator(iSelectedImageSet)->GetIMGByIterator(iSelectedFrame);
		...
		std::string strPath = GV->editState->hDataCtrl->GetFeed(DB_FEED_CUSTOM)->GetAbsoluteLocation() + "/" + fimg->GetFile().strPath;
		strPath = strPath.substr(0, strPath.rfind('/'));
		SHR::RemoveDirR(strPath.c_str());
	}
	else if (actionEvent.getSource() == butImageDetails) {
		GV->StateMgr->Push(new State::ImageDetails((cImage*)(GV->editState->SprBank->GetAssetByIterator(iSelectedImageSet)->GetIMGByIterator(iSelectedFrame))));
	}
}

void winImageSetBrowser::RenderFrameGroup(std::vector<cSprBankAssetIMG*> tiles, int x, int y)
{
	cSprBankAssetIMG * selTile = (iSelectedFrame == -1 ? 0 : GV->editState->SprBank->GetAssetByIterator(iSelectedImageSet)->GetIMGByIterator(iSelectedFrame)),
	...
	for (size_t i = 0; i < tiles.size(); i++) {
		...
		hge->Gfx_RenderLine(drawX + 5 - 2, drawY + 5 - 1, drawX + 5 + (iPreviewDim + 2), drawY + 5 - 1, GV->colLineDark);
		hge->Gfx_RenderLine(drawX + 5 - 1, drawY + 5 + (iPreviewDim + 2), drawX + 5 + (iPreviewDim + 2), drawY + 5 + (iPreviewDim + 2), GV->colLineDark);
		hge->Gfx_RenderLine(drawX + 5 - 1, drawY + 5 - 1, drawX + 5 - 1, drawY + 5 + (iPreviewDim + 2), GV->colLineDark);
		hge->Gfx_RenderLine(drawX + 5 + (iPreviewDim + 2), drawY + 5 - 1, drawX + 5 + (iPreviewDim + 2), drawY + 5 + (iPreviewDim + 2), GV->colLineDark);


		hge->Gfx_RenderLine(drawX + 5 - 1, drawY + 5, drawX + 5 + (iPreviewDim + 1), drawY + 5, colBorder);
		hge->Gfx_RenderLine(drawX + 5, drawY + 5 + (iPreviewDim + 1), drawX + 5 + (iPreviewDim + 1), drawY + 5 + (iPreviewDim + 1), colBorder);
		hge->Gfx_RenderLine(drawX + 5, drawY + 5, drawX + 5, drawY + 5 + (iPreviewDim + 1), colBorder);
		hge->Gfx_RenderLine(drawX + 5 + (iPreviewDim + 1), drawY + 5, drawX + 5 + (iPreviewDim + 1), drawY + 5 + (iPreviewDim + 1), colBorder);

		hgeSprite * spr = (tile->IsLoaded() ? tile->GetImage() : GV->sprSmiley);

		spr->SetColor(0xFFFFFFFF);

		int clipY = std::max(drawY + 5, tilePickY), clipH = std::min(drawY + 5 + iPreviewDim, tilePickY + tilePickH) - clipY;
		if (clipH > 0) {
...
int winImageSetBrowser::MouseHandleGroup(std::vector<cSprBankAssetIMG*> tiles, int x, int y, int tilesPerRow)
{
	...
	if (mx > x && mx < x + tilesPerRow * CONST_IMGSETBROWSER_FRAMEICOSIZE - 5 && my > y + 5) {
		...
		if (imousetile < GV->editState->SprBank->GetAssetByIterator(iSelectedImageSet)->GetSpritesCount() && mx >(x + dx * CONST_IMGSETBROWSER_FRAMEICOSIZE + 5) && mx < (x + (dx + 1)*CONST_IMGSETBROWSER_FRAMEICOSIZE - 10) && my >(y + dy * (CONST_IMGSETBROWSER_FRAMEICOSIZE + 20) + 5))
			return imousetile;
	}
	return -1;
}
