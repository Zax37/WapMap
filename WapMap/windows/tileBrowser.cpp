...
void winTileBrowser::Think()
{
    ...
    if( mx > tilePickX && my > tilePickY && mx < tilePickX+tilePickW && my < tilePickY+tilePickH ){
     if( bSingleGroup ){
      iHighlightedT = MouseHandleGroup(*hSingleGroup, tilePickX+borderoffset, tilePickY+10-scroll, tilesPerRow);
     }else{
      iHighlightedT = -1;
      int ypos = 0;
      for(int i=0;i<3;i++)
...
void winTileBrowser::Draw(int piCode)
{
    ...
    for(int i=startset;i<hTileset->GetSetsCount()&&i<startset+(saTileSets->getHeight()/68)+2;i++){
     ...
     if( i == iSelectedTileSet || i == iHighlightedTS ){
      ...
      q.tex = 0;
      SHR::SetQuad(&q, (i == iSelectedTileSet ? 0x331796ff : 0x33a1a1a1), drawX, drawY, drawX+202, drawY+68);
...
void winTileBrowser::action(const ActionEvent& actionEvent)
{
    if( actionEvent.getSource() == butBrowseFolder ){
     ...
     GV->StateMgr->Push(new State::ImageImport(State::ImageImportTile, GV->editState->hTileset->GetSet(iSelectedTileSet)));
     ...
     std::string strPath = GV->editState->hDataCtrl->GetFeed(DB_FEED_CUSTOM)->GetAbsoluteLocation() + "/TILES/" + ts->GetName();
...
{
    cTile * selTile = (iSelectedTile == -1 ? 0 : GV->editState->hTileset->GetSet(iSelectedTileSet)->GetTileByIterator(iSelectedTile)),
...
int winTileBrowser::MouseHandleGroup(std::vector<cTile*> tiles, int x, int y, int tilesPerRow)
{
    ...
    if( mx > x && mx < x+tilesPerRow*60-5 && my > y+5 ){
     ...
     if( imousetile < GV->editState->hTileset->GetSet(iSelectedTileSet)->GetTilesCount() && mx > (x+dx*60+5) && mx < (x+dx*60+55) && my > (y+dy*80+5) )
      return imousetile;
    }
    return -1;
}
