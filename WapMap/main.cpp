...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR cmdline, int)
{
	...
	if (strcmp(fn, EXENAME) != 0) {
		MessageBox(NULL, "This installation of WM is broken. Please download and install fresh version.", "WapMap", MB_OK | MB_ICONERROR);
		return 1;
	}
	delete[] fn;
#endif
	...
	hge->System_SetState((hgeBoolState)wmHwndState::WM_FILEDROPFUNC, FileDroppedFunc);
	...
	hge->System_SetState((hgeBoolState)wmHwndState::WM_ACCEPTFILES, 1);
	...
	//if( !strncmp(cmdline, "-updateBM", 9) )
	hge->System_SetState((hgeBoolState)wmHwndState::WM_WINDOWCAPTION, 0);
