...
void cRulers::Think()
{
    if( GV->editState->hParser == NULL || !bVisible || GV->editState->iActiveTool != EWW_TOOL_NONE || !GV->editState->bShowGuideLines ) return;
    float mx, my;
    ...
    if( GV->editState->iManipulatedGuide != -1 ){
     if( hge->Input_GetKeyState(HGEK_LBUTTON) ){
      ...
      GV->editState->MDI->GetActiveDoc()->vGuides.erase(GV->editState->MDI->GetActiveDoc()->vGuides.begin()+GV->editState->iManipulatedGuide);
...
void cRulers::Render()
{
    ...
    SHR::SetQuad(&q, 0xFF303030, drX, drY, drX+(rl ? 17 : GV->editState->vPort->GetWidth()), drY+(rl ? GV->editState->vPort->GetHeight() : 17));
    hge->Gfx_RenderQuad(&q);
    ...
    for(int i=0;i< (rl ?  GV->editState->vPort->GetHeight()/(step*zoom)
     ...
     int constoff = ((rl ? GV->editState->fCamY : GV->editState->fCamX) < 0)*(rl ? GV->editState->fCamY : GV->editState->fCamX)*-1;
     ...
     if( type == 3 ){
      ...
      GV->fntMinimal->printf(linex+(rl ? -16 : 3), liney+(rl ? -2 : -20), HGETEXT_LEFT, text);
     }else{
      int h = 3;
      if( type == 1 ) h = 10;
      else if( type == 2 ) h = 17;
      ...
      if( type == 2 && step < 250 ){
       ...
       GV->fntMinimal->printf(linex+(rl ? -16 : 3), liney+(rl ? -3 : -20), HGETEXT_LEFT, text);
      }
     }
    }
    hge->Gfx_SetClipping();
    ...
    if( GV->editState->iManipulatedGuide != -1 ){
     ...
     if( gl.iPos < 0 && gl.iPos != -100 ){
      if( gl.bOrient == GUIDE_HORIZONTAL ){
       hge->Gfx_RenderLine(19, drawY-gl.iPos, 18+GV->editState->vPort->GetWidth(), drawY-gl.iPos, 0xFF30DBD9);
      }else{
       hge->Gfx_RenderLine(-gl.iPos, drawY+18, -gl.iPos, drawY+18+GV->editState->vPort->GetHeight(), 0xFF30DBD9);
      }
     }
    }
}
