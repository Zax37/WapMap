...
void cMruList::Reload()
{
    ...
    if( GV->iOS == OS_VISTA || GV->iOS == OS_7 ){
     RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ComDlg32\\OpenSavePidlMRU\\WWD", 0, KEY_QUERY_VALUE, &key);
     char mrulist[100];
     DWORD size = 100;
    ...
    }else if( RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ComDlg32\\OpenSaveMRU\\WWD", 0, KEY_QUERY_VALUE, &key) != ERROR_SUCCESS ){
...
void cMruList::SaveToRegistry(const char * szNew)
{
    HKEY key;
    if( GV->iOS == OS_VISTA || GV->iOS == OS_7 ){
     RegOpenKeyEx(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\ComDlg32\\OpenSavePidlMRU\\WWD", 0, KEY_QUERY_VALUE|KEY_SET_VALUE, &key);
     char mrulist[100];
     DWORD size = 100;
     if( RegQueryValueEx(key, "MRUListEx", NULL, NULL, (LPBYTE)(&mrulist), &size) != ERROR_SUCCESS ){
      ...
      int regdata[vals.size()+1];
      for(int i=0;i<vals.size();i++)
       regdata[i] = vals[i].first;
      regdata[vals.size()] = 0xFFFFFFFF;

      if( RegSetValueEx(key, "MRUListEx", NULL, REG_BINARY, (const BYTE*)&regdata, DWORD((vals.size()+1)*4)) != ERROR_SUCCESS ){
       RegCloseKey(key);
