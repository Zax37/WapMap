...
State::LoadMap::LoadMap(void * ptr, int size, bool addplanes, int mw, int mh, char name[64], char author[64])
{
...
void State::LoadMap::Init()
{
     ...
     if( !GV->editState->ValidateLevelName(hParser->GetName(), 0) && alt_ptr == 0 ){
      if( MessageBox(hge->System_GetState(HGE_HWND), GETL2S("Various", "BadNamePrompt"), hParser->GetName(), MB_YESNO|MB_ICONWARNING) == IDYES ){
     ...
     if( dd->iWapMapBuild != -1 ){
      GV->Console->Printf("Found extended meta tags (generated by WM v~y~%s ~w~b~y~%d~w~).", dd->strWapMapVersion.c_str(), dd->iWapMapBuild);
     }
     ...
     dd->hDataCtrl = new cDataController(std::string(GV->szClawPath ? GV->szClawPath : ""), std::string(szDir ? szDir : ""), std::string(szFilename));
     GV->Console->Printf(" ~y~REZ file:       ~g~%s", dd->hDataCtrl->GetFeed(DB_FEED_REZ) == 0 ? "~r~[not mounted]"
                                                                                      : dd->hDataCtrl->GetFeed(DB_FEED_REZ)->GetAbsoluteLocation().c_str());
     GV->Console->Printf(" ~y~Disc directory: ~g~%s", dd->hDataCtrl->GetFeed(DB_FEED_DISC) == 0 ? "~r~[not mounted]"
                                                                                      : dd->hDataCtrl->GetFeed(DB_FEED_DISC)->GetAbsoluteLocation().c_str());
     GV->Console->Printf(" ~y~Custom dir:     ~g~%s", dd->hDataCtrl->GetFeed(DB_FEED_CUSTOM) == 0 ? "~r~[not mounted]"
                                                                                      : dd->hDataCtrl->GetFeed(DB_FEED_CUSTOM)->GetAbsoluteLocation().c_str());
     ...
     GV->Console->Printf("~g~Map loaded ~w~(~y~%s~w~ by ~y~%s~w~, base level %d).", hParser->GetName(), hParser->GetAuthor(), hParser->GetBaseLevel());
