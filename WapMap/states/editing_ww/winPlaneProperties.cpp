...
void State::EditingWW::SavePlaneProperties() {
    ...
    if (bAddingNew) {
        ...
        if (atoi(tfpmPlaneSizeX->getText().c_str()) != pl->GetPlaneWidth() ||
            ...
            bChange = 1;
            bDimChange = 1;
            pl->ResizeAnchor(atoi(tfpmPlaneSizeX->getText().c_str()), atoi(tfpmPlaneSizeY->getText().c_str()),
                             ipmAnchor);
        }

        bool bischange = 0;
        if (strcmp(ddpmTileSet->getListModel()->getElementAt(ddpmTileSet->getSelected()).c_str(), pl->GetImageSet(0)) !=
            0) {
            pl->ClearImageSets();
            pl->AddImageSet(ddpmTileSet->getListModel()->getElementAt(ddpmTileSet->getSelected()).c_str());
            bChange = 1;
    ...
    /*if (bDimChange) {
        if (pl->GetPlaneWidthPx() <= 2048 && pl->GetPlaneHeightPx() <= 2048) {
            ...
            if (hPlaneData[lbpmPlanes->getSelected()]->hRB != NULL) {
                delete hPlaneData[lbpmPlanes->getSelected()]->hRB;
                GV->Console->Printf("Deleting buffer for plane '~y~%s~w~' (%dx%d).", pl->GetName(),
                                    pl->GetPlaneWidthPx(), pl->GetPlaneHeightPx());
            }
    ...
    if (cbpmFlagMainPlane->isSelected() != pl->GetFlag(WWD::Flag_p_MainPlane)) {
        ...
        if (bDo) {
            ...
            if (hParser->GetStartX() > pl->GetPlaneWidthPx() || hParser->GetStartY() > pl->GetPlaneHeightPx()) {
                char tmp[256];
                sprintf(tmp, GETL2S("PlaneProperties", "StartPosMoved"), hParser->GetStartX(), hParser->GetStartY(),
                        pl->GetPlaneWidthPx() / 2,
