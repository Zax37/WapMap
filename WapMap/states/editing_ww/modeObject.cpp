...
bool State::EditingWW::ObjectThink(bool pbConsumed)
{
    ...
    if( iActiveTool == EWW_TOOL_EDITOBJ && hEditObj->Kill() ){
     ...
     if( hEditObj->iType == ObjEdit::enText ){
    ...
    hge->Input_GetMousePos(&mx, &my);
     if( iActiveTool == EWW_TOOL_ALIGNOBJ ){
      if( vPort->GetWidget()->isMouseOver() && !pbConsumed && hge->Input_KeyDown(HGEK_LBUTTON) ){
       ...
       std::vector<WWD::Object*> picked = hPlaneData[GetActivePlaneID()]->ObjectData.hQuadTree->GetObjectsByArea(x, y, 1, 1);
       if( picked.size() != 0 ){
        ...
        if( aligntoobj != 0 ){
         for(int i=0;i<vObjectsPicked.size();i++){
          if( bObjectAlignAxis ) vObjectsPicked[i]->SetParam(WWD::Param_LocationY, aligntoobj->GetParam(WWD::Param_LocationY));
          else                   vObjectsPicked[i]->SetParam(WWD::Param_LocationX, aligntoobj->GetParam(WWD::Param_LocationX));
      ...
      /*if( fObjPickLastMx != mx || fObjPickLastMy != my ){
       ...
       }else if( hge->Input_GetKeyState(HGEK_LBUTTON) && bObjDragSelection && vPort->GetWidget()->isMouseOver() && !pbConsumed ){
        ...
        if( actx == iObjDragOrigX && acty == iObjDragOrigY && vObjectsHL.size() > 1 ){
         ...
         for(int i=0;i<vObjectsPicked.size();i++)
          for(int y=0;y<vObjectsHL.size();y++)
           if( vObjectsPicked[i] == vObjectsHL[y] )
            selid = y;
         if( !hge->Input_GetKeyState(HGEK_CTRL) && !hge->Input_GetKeyState(HGEK_SHIFT) && !hge->Input_GetKeyState(HGEK_ALT) )
          vObjectsPicked.clear();
         if( selid == -1 ){
       ...
       if( hge->Input_KeyUp(HGEK_LBUTTON) ){
        ...
        if( vObjectsPicked.size() == 1 ){
         ...
         std::vector<WWD::Object*> mouseobj = hPlaneData[GetActivePlaneID()]->ObjectData.hQuadTree->GetObjectsByArea(mwx, mwy, mwx+1, mwy+1);
         bool found = 0;
         for(int i=0;i<mouseobj.size();i++)
          if( mouseobj[i] == vObjectsPicked[0] ){ found = 1; break; }
         if( found ){
          ...
          if( fDoubleClickTimer >= 0 && fDoubleClickTimer < 0.5 && iDoubleClickX == mx && iDoubleClickY == my ){
      ...
      if( hge->Input_KeyDown(HGEK_LBUTTON) || hge->Input_KeyDown(HGEK_RBUTTON) || hge->Input_KeyUp(HGEK_RBUTTON) ){
       ...
       if( ( hge->Input_KeyDown(HGEK_RBUTTON) || hge->Input_KeyUp(HGEK_RBUTTON) ) && !bObjDragSelection ){
        std::vector<WWD::Object*> selobjs = hPlaneData[GetActivePlaneID()]->ObjectData.hQuadTree->GetObjectsByArea(Scr2WrdX(GetActivePlane(), mx), Scr2WrdY(GetActivePlane(), my), 1, 1);
          ...
          if( hge->Input_KeyDown(HGEK_RBUTTON) ){
           ...
           if( vObjectsPicked.size() == 1 ){
            if( vObjectsPicked[0] == hStartingPosObj )
             ...
             if( !strcmp(vObjectsPicked[0]->GetLogic(), "CustomLogic") && hCustomLogics->GetLogicByName(vObjectsPicked[0]->GetName()) != 0 ){
              objContext->AddElement(OBJMENU_EDITLOGIC, GETL2S("Various", "ContextEditLogic"), GV->sprIcons16[Icon16_Code],
                                     objContext->GetElementByID(OBJMENU_PROPERTIES));
             }
             if( spec ){
              objContext->AddElement(OBJMENU_SPECIFICPROP, GETL2S("Various", "ContextAdv"), GV->sprIcons16[Icon16_PropertyTree],
                                     objContext->GetElementByID(OBJMENU_PROPERTIES));
        ...
        if( !objContext->isVisible() && !bObjDragSelection ){
         if( bOpenObjContext && hge->Input_KeyUp(HGEK_RBUTTON) && conMain->getWidgetAt(mx, my) == vPort->GetWidget() ){
          objContext->setVisible(1);
          bOpenObjContext = 0;
      ...
      if( (fObjPickLastMx != Scr2WrdX(GetActivePlane(), mx) || fObjPickLastMy != Scr2WrdY(GetActivePlane(), my)) &&
       ...
       for(int i=0;i<vObjectsPicked.size();i++){
        ...
        int basex = vObjectsPicked[i]->GetParam(WWD::Param_LocationX), basey = vObjectsPicked[i]->GetParam(WWD::Param_LocationY);
      ...
      if( hge->Input_KeyDown(HGEK_LBUTTON) ){
       ...
       for(int i=0;i<vObjectsPicked.size();i++){
        if( bRecalc ){
         int diffx = GetUserDataFromObj(vObjectsPicked[i])->GetX()-vObjectsPicked[i]->GetParam(WWD::Param_LocationX),
             diffy = GetUserDataFromObj(vObjectsPicked[i])->GetY()-vObjectsPicked[i]->GetParam(WWD::Param_LocationY);
         if( !strcmp(vObjectsPicked[i]->GetLogic(), "Shake") ){
          ...
          if( vObjectsPicked[i]->GetParam(WWD::Param_MinX) != 0 )
           vObjectsPicked[i]->SetParam(WWD::Param_MinX, vObjectsPicked[i]->GetParam(WWD::Param_MinX)+diffx);
          if( vObjectsPicked[i]->GetParam(WWD::Param_MinY) != 0 )
           vObjectsPicked[i]->SetParam(WWD::Param_MinY, vObjectsPicked[i]->GetParam(WWD::Param_MinY)+diffy);
          if( vObjectsPicked[i]->GetParam(WWD::Param_MaxX) != 0 )
           vObjectsPicked[i]->SetParam(WWD::Param_MaxX, vObjectsPicked[i]->GetParam(WWD::Param_MaxX)+diffx);
          if( vObjectsPicked[i]->GetParam(WWD::Param_MaxY) != 0 )
           vObjectsPicked[i]->SetParam(WWD::Param_MaxY, vObjectsPicked[i]->GetParam(WWD::Param_MaxY)+diffy);
         }
        }
        ...
        if( !hge->Input_GetKeyState(HGEK_LBUTTON) ){
         ...
         if( distance > sliobrDistance->getValue() ){
          ...
          for(int i=0;i<vObjectsBrushCB.size();i++){
           ...
           int diffx = vObjectsBrushCB[i]->GetParam(WWD::Param_LocationX)-vObjectsBrushCB[0]->GetParam(WWD::Param_LocationX),
               diffy = vObjectsBrushCB[i]->GetParam(WWD::Param_LocationY)-vObjectsBrushCB[0]->GetParam(WWD::Param_LocationY);
...
bool State::EditingWW::IsEditableObject(WWD::Object * obj, ObjEdit::cObjEdit ** hEdit)
{
    ...
    if( strstr(loweredlogic, "curse") ){
     ...
     ret = 1;
    }else if( !strcmp(obj->GetLogic(), "AniRope") || !strcmp(obj->GetLogic(), "DoNothing") && !strcmp(obj->GetImageSet(), "LEVEL_ROPE") ){
     if( hEdit != 0 ) *hEdit = new ObjEdit::cEditObjRope(obj,this);
     ret = 1;
    }else if( !strcmp(obj->GetLogic(), "HealthPowerup") &&
              !(!strcmp(obj->GetImageSet(), "GAME_CATNIPS_NIP1") || !strcmp(obj->GetImageSet(), "GAME_CATNIPS_NIP2")) ||
...
void State::EditingWW::OpenObjectEdit(WWD::Object * obj)
{
    if( IsEditableObject(obj, &hEditObj) ){
     GV->Console->Printf("~w~Opening EasyEdit tool for object #~y~%d ~w~(~y~%s~w~).", obj->GetParam(WWD::Param_ID), obj->GetLogic());
     SetTool(EWW_TOOL_EDITOBJ);
    }else{
     GV->Console->Printf("~r~Tried to open EasyEdit tool for object #~y~%d ~w~(~y~%s~w~), which is not editable.", obj->GetParam(WWD::Param_ID), obj->GetLogic());
    }
}
...
void State::EditingWW::CreateObjectWithEasyEdit(gcn::Widget * widg)
{
    ...
    if( widg == hmbObject->butIconCurse ){
     ...
     if(      hParser->GetBaseLevel() == 1  )  obj->SetImageSet("LEVEL_ELEVATORS");
              ...
              hParser->GetBaseLevel() == 3  )  obj->SetImageSet("LEVEL_ELEVATOR1");
              ...
              hParser->GetBaseLevel() >  10 )  obj->SetImageSet("LEVEL_ELEVATOR");
     ...
     obj->SetAttackRect(WWD::Rect(std::max(obj->GetParam(WWD::Param_LocationX)-100, 1),
                                  ...
                                  std::min(obj->GetParam(WWD::Param_LocationX)+100, GetActivePlane()->GetPlaneWidthPx()-1),
                                  std::min(obj->GetParam(WWD::Param_LocationY)+100, GetActivePlane()->GetPlaneHeightPx()-1)));
     ...
     obj->SetUserRect(0, WWD::Rect(std::max(obj->GetParam(WWD::Param_LocationX)-100, 1),
                                   std::min(obj->GetParam(WWD::Param_LocationY)+100, GetActivePlane()->GetPlaneHeightPx()-1),
                                   std::min(obj->GetParam(WWD::Param_LocationX)+100, GetActivePlane()->GetPlaneWidthPx()-1),
                                   std::min(obj->GetParam(WWD::Param_LocationY)+300, GetActivePlane()->GetPlaneHeightPx()-1)));
      ...
      if( !cbObjSearchCaseSensitive->isSelected() ) comp2 = SHR::ToLower(szObjSearchBuffer);
        for(int i=0;i<GetActivePlane()->GetObjectsCount();i++){
         ...
         if( strstr(comp, comp2) != NULL ){
          vObjSearchResults.push_back(std::pair<int,int>(i, GetActivePlane()->GetObjectByIterator(i)->GetParam(WWD::Param_ID)));
