...
void State::EditingWW::DrawTileAtrib(WWD::TileAtrib * atrib, float posx, float posy, float width, float height)
{
	...
	if (atrib->GetType() == WWD::AtribType_Single) {
		...
		q.v[1].x = q.v[0].x + wid; q.v[1].y = q.v[0].y;
		q.v[2].x = q.v[1].x; q.v[2].y = q.v[0].y + hei;
		...
		if (atrib->GetAtribInside() != WWD::Atrib_Clear) {
			...
			q.v[0].x = posx + mask.x1*width;  q.v[0].y = posy + mask.y1*height;
			q.v[1].x = posx + (mask.x2 + 1)*width; q.v[1].y = q.v[0].y;
			q.v[2].x = q.v[1].x;              q.v[2].y = posy + (mask.y2 + 1)*height;
			q.v[3].x = q.v[0].x;              q.v[3].y = q.v[2].y;
			hge->Gfx_RenderQuad(&q);
		}
		if (atrib->GetAtribOutside() != WWD::Atrib_Clear) {
			...
			q.v[0].x = posx;                                  q.v[0].y = posy;
			q.v[1].x = q.v[0].x + wid;  q.v[1].y = q.v[0].y;
			q.v[2].x = q.v[1].x;                              q.v[2].y = posy + mask.y1*height;
			q.v[3].x = q.v[0].x;                              q.v[3].y = q.v[2].y;
			hge->Gfx_RenderQuad(&q);
			//lower
			q.v[0].x = posx;                                 q.v[0].y = posy + (mask.y2 + 1)*height;
			q.v[1].x = q.v[0].x + wid; q.v[1].y = q.v[0].y;
			q.v[2].x = q.v[1].x;                             q.v[2].y = posy + hei;
			q.v[3].x = q.v[0].x;                             q.v[3].y = q.v[2].y;
			hge->Gfx_RenderQuad(&q);
			//left
			q.v[0].x = posx;               q.v[0].y = posy + mask.y1*height;
			q.v[1].x = posx + mask.x1*width; q.v[1].y = q.v[0].y;
			q.v[2].x = q.v[1].x;           q.v[2].y = posy + (mask.y2 + 1)*height;
			q.v[3].x = q.v[0].x;           q.v[3].y = q.v[2].y;
			hge->Gfx_RenderQuad(&q);
			//right
			q.v[0].x = posx + (mask.x2 + 1)*width;               q.v[0].y = posy + mask.y1*height;
			q.v[1].x = posx + wid; q.v[1].y = q.v[0].y;
			q.v[2].x = q.v[1].x;                         q.v[2].y = posy + (mask.y2 + 1)*height;
			q.v[3].x = q.v[0].x;                         q.v[3].y = q.v[2].y;
			hge->Gfx_RenderQuad(&q);
		}
...
void State::EditingWW::DrawDB()
{
	...
	if (tabindex == 1) {
		...
		if (dbAssetsImg->getSelected() != -1 && dbFramesImg->getSelected() != -1) {
			...
			for (int i = 0; i < 8; i++) {
				hge->Gfx_RenderLine(dx + 494, dy + 100 + i * 20, dx + winDB->getWidth() - 5, dy + 100 + i * 20, GV->colLineBright);
				hge->Gfx_RenderLine(dx + 494, dy + 99 + i * 20, dx + winDB->getWidth() - 5, dy + 99 + i * 20, GV->colLineDark);
			}
			...
			cSprBankAssetIMG * img = SprBank->GetAssetByIterator(dbAssetsImg->getSelected())->GetIMGByIterator(dbFramesImg->getSelected());

			GV->fntMyriad13->printf(dx + 650, dy + 99, HGETEXT_RIGHT, "~y~%d", dbFramesImg->getSelected());
			GV->fntMyriad13->printf(dx + 650, dy + 119, HGETEXT_RIGHT, "~y~%d", img->GetID());
			...
			if (cbdbisBorder->isSelected()) {
				...
				hge->Gfx_RenderLine(sprx - fsprw - hsx, spry - fsprh - hsy, sprx + fsprw - hsx, spry - fsprh - hsy, 0xFFFF0000);
				hge->Gfx_RenderLine(sprx + fsprw - hsx, spry - fsprh - hsy, sprx + fsprw - hsx, spry + fsprh - hsy, 0xFFFF0000);
				hge->Gfx_RenderLine(sprx + fsprw - hsx, spry + fsprh - hsy, sprx - fsprw - hsx, spry + fsprh - hsy, 0xFFFF0000);
				hge->Gfx_RenderLine(sprx - fsprw - hsx, spry + fsprh - hsy, sprx - fsprw - hsx, spry - fsprh - hsy, 0xFFFF0000);
			}
	...
	if (!sadbFramesImg->isVisible() && tabindex == 1 ||
		...
		GV->fntMyriad13->printf(dx + 700, dy + 100, HGETEXT_CENTER, GETL(Lang_SelectResourceFromList));
	}

	if (tabindex == 2 && dbAssetsAni->getSelected() != -1) {
		if (dbFramesAni->getSelected() == -1 || dbFramesAni->getSelected() >= dbFramesAni->getListModel()->getNumberOfElements())
		...
		GV->fntMyriad13->printf(dx + 780, dy + 220, HGETEXT_RIGHT, "%d/%dms (%d)", int(fdbAniTimer), ani->GetFrame(dbFramesAni->getSelected())->GetDuration(), ani->GetFrame(dbFramesAni->getSelected())->GetImageID());
		...
		if (cbdbBorderAni->isSelected()) {
			...
			hge->Gfx_RenderLine(dx + 400 - sprw - hsx, dy + 375 - sprh - hsy, dx + 400 + sprw - hsx, dy + 375 - sprh - hsy, 0x77FF0000);
			hge->Gfx_RenderLine(dx + 400 + sprw - hsx, dy + 375 - sprh - hsy, dx + 400 + sprw - hsx, dy + 375 + sprh - hsy, 0x77FF0000);
			hge->Gfx_RenderLine(dx + 400 + sprw - hsx, dy + 375 + sprh - hsy, dx + 400 - sprw - hsx, dy + 375 + sprh - hsy, 0x77FF0000);
			hge->Gfx_RenderLine(dx + 400 - sprw - hsx, dy + 375 + sprh - hsy, dx + 400 - sprw - hsx, dy + 375 - sprh - hsy, 0x77FF0000);
...
void State::EditingWW::DrawTileProperties()
{
	...
	hge->Gfx_RenderLine(dx + 8 + !btpZoomTile * 32, dy + 64 + !btpZoomTile * 32, dx + 75 + btpZoomTile * 32 + 32, dy + 64 + !btpZoomTile * 32, GV->colLineDark);
	hge->Gfx_RenderLine(dx + 9 + !btpZoomTile * 32, dy + 63 + !btpZoomTile * 32, dx + 9 + !btpZoomTile * 32, dy + 131 + btpZoomTile * 32 + 32, GV->colLineDark);
	hge->Gfx_RenderLine(dx + 76 + btpZoomTile * 32 + 32, dy + 63 + !btpZoomTile * 32, dx + 76 + btpZoomTile * 32 + 32, dy + 131 + btpZoomTile * 32 + 32, GV->colLineDark);
	hge->Gfx_RenderLine(dx + 8 + !btpZoomTile * 32, dy + 131 + btpZoomTile * 32 + 32, dx + 75 + btpZoomTile * 32 + 32, dy + 131 + btpZoomTile * 32 + 32, GV->colLineDark);

	hge->Gfx_RenderLine(dx + 9 + !btpZoomTile * 32, dy + 65 + !btpZoomTile * 32, dx + 74 + btpZoomTile * 32 + 32, dy + 65 + !btpZoomTile * 32, GV->colLineBright);
	hge->Gfx_RenderLine(dx + 10 + !btpZoomTile * 32, dy + 64 + !btpZoomTile * 32, dx + 10 + !btpZoomTile * 32, dy + 130 + btpZoomTile * 32 + 32, GV->colLineBright);
	hge->Gfx_RenderLine(dx + 75 + btpZoomTile * 32 + 32, dy + 64 + !btpZoomTile * 32, dx + 75 + btpZoomTile * 32 + 32, dy + 130 + btpZoomTile * 32 + 32, GV->colLineBright);
	hge->Gfx_RenderLine(dx + 9 + !btpZoomTile * 32, dy + 130 + btpZoomTile * 32 + 32, dx + 75 + btpZoomTile * 32 + 32, dy + 130 + btpZoomTile * 32 + 32, GV->colLineBright);

	GV->fntMyriad13->Render(dx + 145, dy + 68, HGETEXT_LEFT, GETL(Lang_Width));
	GV->fntMyriad13->Render(dx + 145, dy + 94, HGETEXT_LEFT, GETL(Lang_Height));

	GV->fntMyriad13->Render(dx + 145, dy + 110, HGETEXT_LEFT, GETL(Lang_Type));
	GV->fntMyriad13->Render(dx + 165, dy + 136, HGETEXT_LEFT, GETL(Lang_AtribSingle));
	GV->fntMyriad13->Render(dx + 165, dy + 163, HGETEXT_LEFT, GETL(Lang_AtribDouble));

	GV->fntMyriad13->Render(dx + 5, dy + 205, HGETEXT_LEFT, GETL(Lang_AtribInside));

	GV->fntMyriad13->Render(dx + 125, dy + 205, HGETEXT_LEFT, GETL(Lang_PropClear));
	GV->fntMyriad13->Render(dx + 225, dy + 205, HGETEXT_LEFT, GETL(Lang_PropSolid));

	GV->fntMyriad13->Render(dx + 25, dy + 235, HGETEXT_LEFT, GETL(Lang_PropGround));
	GV->fntMyriad13->Render(dx + 125, dy + 235, HGETEXT_LEFT, GETL(Lang_PropClimb));
	GV->fntMyriad13->Render(dx + 225, dy + 235, HGETEXT_LEFT, GETL(Lang_PropDeath));

	if (rbtpDouble->isSelected()) {
		GV->fntMyriad13->Render(dx + 125, dy + 265, HGETEXT_LEFT, GETL(Lang_PropClear));
		GV->fntMyriad13->Render(dx + 225, dy + 265, HGETEXT_LEFT, GETL(Lang_PropSolid));

		GV->fntMyriad13->Render(dx + 25, dy + 295, HGETEXT_LEFT, GETL(Lang_PropGround));
		GV->fntMyriad13->Render(dx + 125, dy + 295, HGETEXT_LEFT, GETL(Lang_PropClimb));
		GV->fntMyriad13->Render(dx + 225, dy + 295, HGETEXT_LEFT, GETL(Lang_PropDeath));

		GV->fntMyriad13->Render(dx + 5, dy + 265, HGETEXT_LEFT, GETL(Lang_AtribOutside));
		GV->fntMyriad13->Render(dx + 110, dy + 327, HGETEXT_RIGHT, GETL(Lang_Mask));
		GV->fntMyriad13->Render(dx + 150, dy + 327, HGETEXT_LEFT, "x");
		GV->fntMyriad13->Render(dx + 202, dy + 325, HGETEXT_LEFT, "-");
		GV->fntMyriad13->Render(dx + 250, dy + 327, HGETEXT_LEFT, "x");
	}
	...
	if (tile != NULL) {
		if (rbtpDouble->isSelected()) {
			...
			if (btpDragDropMask && !hge->Input_GetKeyState(HGEK_LBUTTON)) {
				...
				int nx, ny;
				if (btpZoomTile) {
					nx = (mx - dx - 10) / 2; ny = (my - dy - 65) / 2;
				}
				else {
					nx = mx - dx - 10 - 32; ny = my - dy - 65 - 32;
				}
				if (nx >= 64) nx = 63;
				if (ny >= 64) ny = 63;
				if (nx < 0) nx = 0;
				if (ny < 0) ny = 0;
				htpWorkingAtrib->SetMask(std::min(itpDDx1, nx), std::min(itpDDy1, ny), std::max(itpDDx1, nx), std::max(itpDDy1, ny));
				char tmp[16];
			...
			for (int x = sx; x < ex; x++) {
						...
						if (x == vTileGhosting[i].x && y == vTileGhosting[i].y && plane == vTileGhosting[i].pl) {
							if (vTileGhosting[i].id >= 0 && vTileGhosting[i].id != plane->GetTile(x, y)->GetID()) {
								...
								if (bDrawTileProperties && pl == GetActivePlaneID()) {
									DrawTileAtrib(vTileGhosting[i].id, posx, posy, 1.0f / (64.0f / (plane->GetTileWidth()*fZoom)), 1.0f / (64.0f / (plane->GetTileHeight()*fZoom)));
								}
								break;
							}
								...
								q.blend = BLEND_DEFAULT;
								q.tex = 0;
								q.v[0].col = q.v[1].col = q.v[2].col = q.v[3].col = SETA(hDataCtrl->GetPalette()->GetColor(plane->GetFillColor()), GETA(col));
								q.v[0].z = q.v[1].z = q.v[2].z = q.v[3].z = 1.0f;
								q.v[0].x = posx; q.v[0].y = posy;
				...
				if (!bUsingBuffer)
					if (plane->GetTile(x, y)->IsInvisible()) {
						...
						if (bDrawTileProperties && pl == GetActivePlaneID()) {
							DrawTileAtrib(plane->GetTile(x, y)->GetID(), posx, posy, 1.0f / (64.0f / (plane->GetTileWidth()*fZoom)), 1.0f / (64.0f / (plane->GetTileHeight()*fZoom)));
						}
						rcount++;
					}
				...
				if (highlight) {
					...
					hge->Gfx_RenderLine(posx + 1, posy + th - 1, posx + tw - 1, posy + th - 1, TILE_HIGHLIGHT_COLOR_SOLID);
					hge->Gfx_RenderLine(posx + 1, posy + 1, posx + 1, posy + th - 1, TILE_HIGHLIGHT_COLOR_SOLID);
					hge->Gfx_RenderLine(posx + tw - 1, posy + 1, posx + tw - 1, posy + th - 1, TILE_HIGHLIGHT_COLOR_SOLID);
					...
					GV->fntMyriad13->SetColor(0xFF000000);
					GV->fntMyriad13->Render(posx + tw / 2 + 1, posy + tw * 0.75 + 1 - 20, HGETEXT_CENTER, text);
					GV->fntMyriad13->SetColor(0xFFFFFFFF);
					GV->fntMyriad13->Render(posx + tw / 2, posy + tw * 0.75 - 20, HGETEXT_CENTER, text);
				}

				if (bUsingBuffer) continue;

				if (hPlaneData[pl]->bDrawBoundary &&
					((x%plane->GetPlaneWidth()) == plane->GetPlaneWidth() - 1 && (plane->GetFlags() & WWD::Flag_p_YWrapping) != 0 ||
					(x%plane->GetPlaneWidth()) == plane->GetPlaneWidth() - 2 && (plane->GetFlags() & WWD::Flag_p_YWrapping) == 0))
					hge->Gfx_RenderLine(int(posx + plane->GetTileWidth()*fZoom), 0, int(posx + plane->GetTileWidth()*fZoom), endplaney, ARGB(GETA(col), 255, 0, 255));
				else if (hPlaneData[pl]->bDrawGrid)
					hge->Gfx_RenderLine(int(posx + plane->GetTileWidth()*fZoom), 0, int(posx + plane->GetTileWidth()*fZoom), endplaney, ARGB(GETA(col), 255, 255, 255));
			}
			if (bUsingBuffer) continue;
			if (hPlaneData[pl]->bDrawBoundary &&
				((y%plane->GetPlaneHeight()) == plane->GetPlaneHeight() - 1 && (plane->GetFlags() & WWD::Flag_p_YWrapping) != 0 ||
				(y%plane->GetPlaneHeight()) == plane->GetPlaneHeight() - 2 && (plane->GetFlags() & WWD::Flag_p_YWrapping) == 0))
				hge->Gfx_RenderLine(0, int(posy + plane->GetTileHeight()*fZoom), endplanex, int(posy + plane->GetTileHeight()*fZoom), ARGB(GETA(col), 255, 0, 255));
			else if (hPlaneData[pl]->bDrawGrid)
				hge->Gfx_RenderLine(0, int(posy + plane->GetTileHeight()*fZoom), endplanex, int(posy + plane->GetTileHeight()*fZoom), ARGB(GETA(col), 255, 255, 255));
		}
...
void State::EditingWW::RenderToViewportBuffer()
{
	...
	for (int i = 0; i < hParser->GetPlanesCount(); i++) {
		...
		if (hPlaneData[i]->bDrawObjects && hPlaneData[i]->ObjectData.hQuadTree != NULL) {
			std::vector<WWD::Object*> list = hPlaneData[i]->ObjectData.hQuadTree->GetObjectsByArea(Scr2WrdX(hParser->GetPlane(i), vPort->GetX()),
...
void State::EditingWW::DrawViewport()
{
	...
	hge->Gfx_SetClipping(hge->System_GetState(HGE_SCREENWIDTH) - 10, 10, 0, hge->System_GetState(HGE_SCREENHEIGHT) - 26);
	...
	GV->hGfxInterface->sprMainFrameRD->Render(hge->System_GetState(HGE_SCREENWIDTH) - 26, hge->System_GetState(HGE_SCREENHEIGHT) - 26);
	...
	GV->hGfxInterface->sprMainFrameD->RenderStretch(15, hge->System_GetState(HGE_SCREENHEIGHT) - 3,
		hge->System_GetState(HGE_SCREENWIDTH) - 26, hge->System_GetState(HGE_SCREENHEIGHT));
	...
	q.tex = 0;
	q.v[0].z = q.v[1].z = q.v[2].z = q.v[3].z = 1.0f;
	q.v[0].x = 1; q.v[0].y = LAY_APPMENU_Y;
	q.v[1].x = hge->System_GetState(HGE_SCREENWIDTH) - 4; q.v[1].y = LAY_APPMENU_Y;
	q.v[2].x = hge->System_GetState(HGE_SCREENWIDTH) - 4; q.v[2].y = LAY_APPMENU_Y + LAY_APPMENU_H + LAY_MODEBAR_H + 2;
	q.v[3].x = 0; q.v[3].y = LAY_APPMENU_Y + LAY_APPMENU_H + LAY_MODEBAR_H + 3;
	...
	for (int i = 0; i < 2; i++) {
		...
		GV->hGfxInterface->sprAppBar[i]->SetColor(SETA(0xFFFFFFFF, (unsigned char)(125.0f + 130.0f*fAppBarTimers[i] * 5.0f)));
		GV->hGfxInterface->sprAppBar[i]->Render(x, 4);
	}

	hge->Gfx_SetClipping(0, 110, hge->System_GetState(HGE_SCREENWIDTH) - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 113);
	...
	if (hParser != NULL) {
		...
		if (MDI->GetActiveDoc()->vGuides.size() > 0 && bShowGuideLines) {
			...
			for (size_t i = 0; i < MDI->GetActiveDoc()->vGuides.size(); i++) {
				...
				if (i == iManipulatedGuide && gl.iPos >= 0) {
					GV->fntMyriad13->SetColor(0xFF000000);
					GV->fntMyriad13->printf(mx + 14, my + 16, HGETEXT_LEFT, (gl.bOrient == GUIDE_HORIZONTAL ? "Y: %d" : "X: %d"), gl.iPos);
					GV->fntMyriad13->SetColor(0xFFFFFFFF);
					GV->fntMyriad13->printf(mx + 15, my + 15, HGETEXT_LEFT, (gl.bOrient == GUIDE_HORIZONTAL ? "Y: %d" : "X: %d"), gl.iPos);
				}
			}
		}
		...
		if (iMode == EWW_MODE_OBJECT && iActiveTool == EWW_TOOL_NONE && hge->Input_GetKeyState(HGEK_TAB) && vObjectClipboard.size() != 0) {
		...
		if (winDuplicate->isVisible() && atoi(tfdTimes->getText().c_str()) > 0) {
			...
			for (int i = 0; i < count; i++) {
				if (objdSource != NULL) {
					RenderObject(objdSource, Wrd2ScrX(GetActivePlane(), GetUserDataFromObj(objdSource)->GetX() + offx * (i + 1)),
						Wrd2ScrY(GetActivePlane(), GetUserDataFromObj(objdSource)->GetY() + offy * (i + 1)),
						0x77FFFFFF);
				}
				else {
					for (int y = 0; y < vObjectsPicked.size(); y++) {
						RenderObject(vObjectsPicked[y], Wrd2ScrX(GetActivePlane(), GetUserDataFromObj(vObjectsPicked[y])->GetX() + offx * (i + 1)),
							Wrd2ScrY(GetActivePlane(), GetUserDataFromObj(vObjectsPicked[y])->GetY() + offy * (i + 1)),
							0x77FFFFFF);
					}
				}
			}
		...
		for (int i = 0; i < vObjectsPicked.size(); i++) {
			...
			float posx = (GetUserDataFromObj(obj)->GetX() - hsx)*fZoom, posy = (GetUserDataFromObj(obj)->GetY() - hsy)*fZoom;
			...
			if (vObjectsPicked.size() == 1) {
				...
				if (strstr(obj->GetLogic(), "SoundTrigger") ||
					...
					if (strstr(obj->GetLogic(), "SoundTrigger")) {
						...
						else if (strstr(obj->GetLogic(), "Wide")) { w = 100; h = 32; }
						else if (strstr(obj->GetLogic(), "Tall")) { w = 32;  h = 100; }
						else { w = 64; }
						if (!h) h = w;
						r[0].x2 = r[0].x1 + w;
						r[0].y2 = r[0].y1 + h;
					...
					for (int i = 0; i < 3; i++) {
						...
						RenderAreaRect(r[i], WWD::Rect(1, 1, 1, 1), 0, bmulticolor ? dwCols[i] : 0xAAFFFFFF, 1, bmulticolor ? dwfCols[i] : 0x20FFFFFF);
						if (bmulticolor) {
							GV->fntMyriad13->printf(cx + 11, cy + 11, HGETEXT_LEFT, "~l~%s", GETL2S("EditObj_Dialog", szRectID[i]));
							GV->fntMyriad13->SetColor(SETA(dwCols[i], 255));
							GV->fntMyriad13->printf(cx + 10, cy + 10, HGETEXT_LEFT, "%s", GETL2S("EditObj_Dialog", szRectID[i]));
						}
					}
					vPort->ClipScreen();
					...
					bool bFill = 1;
					if (!strcmp(vObjectsPicked[0]->GetLogic(), "SpringBoard") || !strcmp(vObjectsPicked[0]->GetLogic(), "GroundBlower")) {
					...
					else if (!strcmp(vObjectsPicked[0]->GetLogic(), "SpotAmbientSound") || !strcmp(vObjectsPicked[0]->GetLogic(), "AmbientPosSound")) {
					...
					if ((minx != 0 || miny != 0 || maxx != 0 || maxy != 0) &&
						...
						if (minx < vPort->GetX() + vPort->GetWidth() ||
							...
							RenderAreaRect(WWD::Rect(mmd_xmin, mmd_ymin, mmd_xmax, mmd_ymax),
								WWD::Rect(minx != 0 && minx > vPort->GetX() - 5 && minx < vPort->GetX() + vPort->GetWidth() + 5,
									miny != 0 && mmd_ymin >= vPort->GetY() && mmd_ymin <= vPort->GetY() + vPort->GetHeight() + 5,
									maxx != 0 && maxx > vPort->GetX() - 5 && maxx < vPort->GetX() + vPort->GetWidth() + 5,
									maxy != 0 && mmd_ymax > vPort->GetY() - 5 && mmd_ymax < vPort->GetY() + vPort->GetHeight() + 5),
								0, 0xAAFFFFFF, bFill, 0x20FFFFFF);

							vPort->ClipScreen();
							if (iSpringboardValue != 0) {
								GV->fntMyriad13->printf(posx - cammx, mmd_ymin - 13, HGETEXT_CENTER, "~l~%s: %d", GETL2S("EditObj_SpringBoard", "OverlayValue"), iSpringboardValue);
								GV->fntMyriad13->printf(posx - cammx - 1, mmd_ymin - 14, HGETEXT_CENTER, "~w~%s: ~y~%d", GETL2S("EditObj_SpringBoard", "OverlayValue"), iSpringboardValue);
							}
						}
					}
				}
				...
				if (!strcmp(obj->GetLogic(), "PathElevator")) {
					...
					for (int s = 0; s < 8; s++) {
						...
						if (bEditMode && (((ObjEdit::cEditObjElevPath*)hEditObj)->GetActualStep() == s ||
							...
							(mx > origx + stepx - spr->GetWidth() / 2 && mx < origx + stepx + spr->GetWidth() / 2 &&
								my > origy + stepy - spr->GetHeight() / 2 && my < origy + stepy + spr->GetHeight() / 2)))
							spr->SetColor(0xFFFFFFFF);
						else
							spr->SetColor(0x77FFFFFF);
						spr->SetFlip(GetUserDataFromObj(obj)->GetFlipX(), GetUserDataFromObj(obj)->GetFlipY(),/*(list[i]->GetDrawFlags()&WWD::Flag_dr_Mirror), (list[i]->GetDrawFlags()&WWD::Flag_dr_Invert),*/ 1);
						...
						RenderArrow(origx + lastx + spacesize * modx*fZoom, origy + lasty + spacesize * mody*fZoom,
							origx + lastx + spacesize * modx*fZoom + (stepxvec - ((fZoom*spacesize*(1 + (mody == 0))*modx))),
							origy + lasty + spacesize * mody*fZoom + (stepyvec - ((fZoom*spacesize*(1 + (modx == 0))*mody))), 0, 0);
						if (arrowbodylen > 0) {
							GV->sprArrowVerticalM->SetColor(0xFFFFFFFF);
							GV->sprArrowVerticalU->SetColor(0xFFFFFFFF);
							GV->fntMyriad13->SetColor(0xFF000000);
							GV->fntMyriad13->printf(origx + lastx + stepxvec / 2, origy + lasty + stepyvec / 2 + 1 - 10, HGETEXT_CENTER, "%s: %d, %d", 0, GETL2S("ObjProp", "Movement"), stepxvec, stepyvec);
							GV->fntMyriad13->SetColor(0xFFFFFFFF);
							GV->fntMyriad13->printf(origx + lastx + stepxvec / 2, origy + lasty + stepyvec / 2 - 10, HGETEXT_CENTER, "%s: ~y~%d~w~, ~y~%d", 0, GETL2S("ObjProp", "Movement"), stepxvec, stepyvec);
						}

						if (!(stepx == 0 && stepy == 0)) {
							// here was bigger font
							spr->RenderEx(origx + stepx, origy + stepy, 0, fZoom);
							GV->fntMyriad13->SetColor(0xFF000000);
							GV->fntMyriad13->printf(origx + stepx + 2, origy + stepy + 2, HGETEXT_LEFT, "%d", s + 1);
							GV->fntMyriad13->SetColor(0xFFFFFFFF);
							GV->fntMyriad13->printf(origx + stepx, origy + stepy, HGETEXT_LEFT, "%d", s + 1);
						}
					}
				}
					...
					if (minx != 0 && maxx != 0 && minx <= maxx || miny != 0 && maxy != 0 && miny <= maxy) {
						...
						if (minx == maxx) { //vertical
							...
							GV->sprArrowVerticalM->RenderEx(x, endy - rpad, 0, fZoom, float(endy - starty - lpad - 37 * fZoom) / 128.0f);
							...
							GV->sprArrowHorizontalC->RenderEx(startx + lpad, y, 0, float(endx - startx - lpad - rpad) / 8.0f, fZoom);
							...
							spr->SetColor(0x77FFFFFF);
							if (bDrawInit) {
								spr->RenderEx(Wrd2ScrX(GetActivePlane(), initx), Wrd2ScrY(GetActivePlane(), inity), 0, fZoom);
							}
							...
							bool bDirVer = (dir <= 3 || dir == 5 || dir > 9), bDirHor = (dir <= 0 || dir == 3 || dir == 6 || dir >= 9);
							bool bPathClosed = 0;
							...
							if (bPathClosed) {
								for (int i = 1; i < vPrevPos.size(); i++) {
									...
									spr->RenderEx(Wrd2ScrX(GetActivePlane(), vPrevPos[i].first + minx), Wrd2ScrY(GetActivePlane(), vPrevPos[i].second + miny), 0, fZoom);
									RenderArrow(Wrd2ScrX(GetActivePlane(), vPrevPos[i - 1].first + minx - arroffx / 2), Wrd2ScrY(GetActivePlane(), vPrevPos[i - 1].second + miny - arroffy / 2),
										Wrd2ScrX(GetActivePlane(), vPrevPos[i].first + minx + arroffx / 2), Wrd2ScrY(GetActivePlane(), vPrevPos[i].second + miny + arroffy / 2),
										1, 0);
								}
							}
							else {
								...
								vec.Clamp(150);
								RenderArrow(Wrd2ScrX(GetActivePlane(), vPrevPos[0].first + minx), Wrd2ScrY(GetActivePlane(), vPrevPos[0].second + miny),
									Wrd2ScrX(GetActivePlane(), vPrevPos[0].first + minx + (arroffx < 0 ? vec.x : -vec.x)*fZoom), Wrd2ScrY(GetActivePlane(), vPrevPos[0].second + miny + (arroffy < 0 ? vec.y : -vec.y)*fZoom),
									0);
							}
						}
					}
					...
					(hEditObj->iType == ObjEdit::enWarp && ((ObjEdit::cEditObjWarp*)hEditObj)->bPick ||
						hEditObj->iType == ObjEdit::enCrate && ((ObjEdit::cEditObjCrate*)hEditObj)->PreviewWarp() ||
						hEditObj->iType == ObjEdit::enStatue && ((ObjEdit::cEditObjStatue*)hEditObj)->PreviewWarp())) {
					int outx, outy;
					bool bdr = 0;
					...
					if (iActiveTool != EWW_TOOL_EDITOBJ || bdr) {
						...
						GV->fntMyriad13->printf(outx + spr2->GetWidth() / 2 + 1, outy + 1, HGETEXT_LEFT, "~l~%s: %d, %d", GETL2S("EditObj_Warp", "SpawnPos"), drx, dry);
						GV->fntMyriad13->printf(outx + spr2->GetWidth() / 2, outy, HGETEXT_LEFT, "~w~%s: ~y~%d~w~, ~y~%d", GETL2S("EditObj_Warp", "SpawnPos"), drx, dry);
					}
				}
				if (iActiveTool == EWW_TOOL_EDITOBJ && (hEditObj->iType == ObjEdit::enPathElevator || hEditObj->IsMovingObject() ||
					hEditObj->iType == ObjEdit::enText)
					|| iActiveTool == EWW_TOOL_MOVEOBJECT)
					bDrawObjProperties = 0;

				if (obj == hStartingPosObj) {
					GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy - sprh, HGETEXT_LEFT, "~l~%s", 0, GETL(Lang_StartingPlace));
					GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy - sprh, HGETEXT_LEFT, "~w~%s~l~", 0, GETL(Lang_StartingPlace));

					GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy - sprh + 15, HGETEXT_LEFT, "~l~X: %d Y: %d", obj->GetParam(WWD::Param_LocationX), obj->GetParam(WWD::Param_LocationY));
					GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy - sprh + 15, HGETEXT_LEFT, "~w~X: ~y~%d ~w~Y: ~y~%d~l~", obj->GetParam(WWD::Param_LocationX), obj->GetParam(WWD::Param_LocationY));
				}
					...
					GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy - sprh, HGETEXT_LEFT, "~l~%s: %d", GETL(Lang_ID), iid);
					GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy - sprh, HGETEXT_LEFT, "~w~%s: ~y~%d", GETL(Lang_ID), iid);
					if (obj->GetName()[0] != '\0' && strcmp(obj->GetLogic(), "CustomLogic") != 0) {
						GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, "~l~%s: %s", GETL(Lang_Name), obj->GetName());
						GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy + ioff - sprh, HGETEXT_LEFT, "~w~%s: ~y~%s", GETL(Lang_Name), obj->GetName());
						ioff += 15;
					}
					...
					if (obj->GetLogic()[0] != '\0') {
						...
						if (customlogic) {
							...
							GV->sprIcons16[Icon16_CrazyHook]->Render(posx - cammx + sprw + 7 + GV->fntMyriad13->GetStringWidth(GETL(Lang_Logic)), posy + vPort->GetY() - cammy + ioff - sprh);
						}
						GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, (customlogic ? "~l~%s:    %s" : "~l~%s: %s"), GETL(Lang_Logic), logicval);
						GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy + ioff - sprh, HGETEXT_LEFT, (customlogic ? "~w~%s:    ~y~%s" : "~w~%s: ~y~%s"), GETL(Lang_Logic), logicval);
						ioff += 15;
					}

					if (obj->GetImageSet()[0] != '\0') {
						GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, "~l~%s: %s", GETL(Lang_Graphic), obj->GetImageSet());
						GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy + ioff - sprh, HGETEXT_LEFT, "~w~%s: ~y~%s", GETL(Lang_Graphic), obj->GetImageSet());
						ioff += 15;
					}

					if (obj->GetAnim()[0] != '\0') {
						GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, "~l~%s: %s", GETL(Lang_Anim), obj->GetAnim());
						GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy + ioff - sprh, HGETEXT_LEFT, "~w~%s: ~y~%s", GETL(Lang_Anim), obj->GetAnim());
						ioff += 15;
					}

					GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, "~l~%s: %dx%d, Z: %d",
						GETL(Lang_Location), obj->GetParam(WWD::Param_LocationX), obj->GetParam(WWD::Param_LocationY),
						obj->GetParam(WWD::Param_LocationZ));
					GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy + ioff - sprh, HGETEXT_LEFT, "~w~%s: ~y~%d~w~x~y~%d~w~, Z: ~y~%d",
						GETL(Lang_Location), obj->GetParam(WWD::Param_LocationX), obj->GetParam(WWD::Param_LocationY),
						obj->GetParam(WWD::Param_LocationZ));
					ioff += 15;
					if (obj->GetParam(WWD::Param_MinX) != 0 || obj->GetParam(WWD::Param_MaxX) != 0) {
						if (obj->GetParam(WWD::Param_MinX) != 0 && obj->GetParam(WWD::Param_MaxX) != 0) {
							GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, "~l~%s: %d. %s: %d.",
								GETL(Lang_MinX), obj->GetParam(WWD::Param_MinX), GETL(Lang_MaxX), obj->GetParam(WWD::Param_MaxX));
							GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy + ioff - sprh, HGETEXT_LEFT, "~w~%s: ~y~%d~w~. %s: ~y~%d~w~.",
								GETL(Lang_MinX), obj->GetParam(WWD::Param_MinX), GETL(Lang_MaxX), obj->GetParam(WWD::Param_MaxX));
						}
						else if (obj->GetParam(WWD::Param_MinX) != 0 && obj->GetParam(WWD::Param_MaxX) == 0) {
							GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, "~l~%s: %d",
								GETL(Lang_MinX), obj->GetParam(WWD::Param_MinX));
							GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy + ioff - sprh, HGETEXT_LEFT, "~w~%s: ~y~%d",
								GETL(Lang_MinX), obj->GetParam(WWD::Param_MinX));
						}
						else if (obj->GetParam(WWD::Param_MinX) == 0 && obj->GetParam(WWD::Param_MaxX) != 0) {
							GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, "~l~%s: %d",
					...
					if (obj->GetParam(WWD::Param_MinY) != 0 || obj->GetParam(WWD::Param_MaxY) != 0) {
						if (obj->GetParam(WWD::Param_MinY) != 0 && obj->GetParam(WWD::Param_MaxY) != 0) {
							GV->fntMyriad13->printf(posx - cammx + sprw + 5, posy + vPort->GetY() + 1 - cammy + ioff - sprh, HGETEXT_LEFT, "~l~%s: %d. %s: %d.",
								GETL(Lang_MinY), obj->GetParam(WWD::Param_MinY), GETL(Lang_MaxY), obj->GetParam(WWD::Param_MaxY));
							GV->fntMyriad13->printf(posx - cammx + sprw + 4, posy + vPort->GetY() - cammy + ioff - sprh, HGETEXT_LEFT, "~w~%s: ~y~%d~w~. %s: ~y~%d~w~.",
								GETL(Lang_MinY), obj->GetParam(WWD::Param_MinY), GETL(Lang_MaxY), obj->GetParam(WWD::Param_MaxY));
						}
					...
					if (!strcmp(obj->GetLogic(), "FrontCrate") || !strcmp(obj->GetLogic(), "Statue") ||
						...
						!(!strcmp(obj->GetLogic(), "Rat") || !strcmp(obj->GetLogic(), "Fish"))) {
						bool singlecrate = !strcmp(obj->GetLogic(), "FrontCrate") || !strcmp(obj->GetLogic(), "BehindCrate");
						...
						for (int y = 0; y < 4; y++)
							for (int x = 0; x < 3; x++) {
								...
								cSprBankAsset * asset = GV->editState->SprBank->GetAssetByID(items[y * 3 + x].first.c_str());
								...
								spr->RenderEx(posx - cammx + sprw + 4 + 52 * x + 24, posy + vPort->GetY() - cammy + ioff - sprh + y * 54 + 24, 0, fScale);
							}
					}
		...
		for (int i = 1; i < m_vMeasurePoints.size(); i++) {
			int x1 = Wrd2ScrX(GetActivePlane(), m_vMeasurePoints[i - 1].first), y1 = Wrd2ScrY(GetActivePlane(), m_vMeasurePoints[i - 1].second),
				x2 = Wrd2ScrX(GetActivePlane(), m_vMeasurePoints[i].first), y2 = Wrd2ScrY(GetActivePlane(), m_vMeasurePoints[i].second);
			hge->Gfx_RenderLine(x1, y1, x2, y2, 0xFFFFFFFF);
			if (cbmeasAbsoluteDistance->isSelected())
				len += DISTANCE(m_vMeasurePoints[i - 1].first, m_vMeasurePoints[i - 1].second, m_vMeasurePoints[i].first, m_vMeasurePoints[i].second);
			else
				len = DISTANCE(m_vMeasurePoints[i - 1].first, m_vMeasurePoints[i - 1].second, m_vMeasurePoints[i].first, m_vMeasurePoints[i].second);
			if (m_vMeasurePoints[i - 1].first < m_vMeasurePoints[i].first && m_vMeasurePoints[i - 1].second < m_vMeasurePoints[i].second || m_vMeasurePoints[i].first < m_vMeasurePoints[i - 1].first && m_vMeasurePoints[i].second < m_vMeasurePoints[i - 1].second)
				GV->fntMyriad13->printf(x1 + (x2 - x1) / 2, y1 + (y2 - y1) / 2 - 20, HGETEXT_LEFT, "~y~%.0f~w~px", len);
			else
				GV->fntMyriad13->printf(x1 + (x2 - x1) / 2, y1 + (y2 - y1) / 2, HGETEXT_LEFT, "~y~%.0f~w~px", len);
		}

		if (iActiveTool == EWW_TOOL_MEASURE && m_vMeasurePoints.size() != 0 && conMain->getWidgetAt(mx, my) == vPort->GetWidget()) {
			...
			if (cbmeasAbsoluteDistance->isSelected())
				len += DISTANCE(m_vMeasurePoints[m_vMeasurePoints.size() - 1].first, m_vMeasurePoints[m_vMeasurePoints.size() - 1].second, Scr2WrdX(GetActivePlane(), scrx), Scr2WrdY(GetActivePlane(), scry));
			else
				len = DISTANCE(m_vMeasurePoints[m_vMeasurePoints.size() - 1].first, m_vMeasurePoints[m_vMeasurePoints.size() - 1].second, Scr2WrdX(GetActivePlane(), scrx), Scr2WrdY(GetActivePlane(), scry));
			if (x1 < scrx && y1 < scry || scrx < x1 && scry < y1)
				GV->fntMyriad13->printf(x1 + (scrx - x1) / 2, y1 + (scry - y1) / 2 - 20, HGETEXT_LEFT, "~y~%.0f~w~px", len);
			else
				GV->fntMyriad13->printf(x1 + (scrx - x1) / 2, y1 + (scry - y1) / 2, HGETEXT_LEFT, "~y~%.0f~w~px", len);
		}
		...
		if (iMode == EWW_MODE_TILE) {
			...
			if (fCamX > maxx && !(GetActivePlane()->GetFlags() & WWD::Flag_p_XWrapping)) {
				hge->Gfx_SetClipping(vPort->GetX() + vPort->GetWidth() - (fCamX - maxx), vPort->GetY(), fCamX - maxx, vPort->GetHeight());
				for (int y = 0; y <= vPort->GetHeight() / 120; y++) {
					GV->sprCheckboard->Render(vPort->GetX() + vPort->GetWidth() - (vPort->GetWidth() % 120), vPort->GetY() + y * 120);
				}
			}
			if (fCamY < 0) {
			...
			if (fCamY > maxy && !(GetActivePlane()->GetFlags() & WWD::Flag_p_YWrapping)) {
				hge->Gfx_SetClipping(vPort->GetX(), vPort->GetY() + vPort->GetHeight() - (fCamY - maxy), vPort->GetWidth(), fCamY - maxy);
				for (int x = 0; x <= vPort->GetWidth() / 120; x++) {
					GV->sprCheckboard->Render(vPort->GetX() + x * 120, vPort->GetY() + vPort->GetHeight() - (vPort->GetHeight() % 120));
				}
			}
		}
		hge->Gfx_SetClipping();
		...
		if (bDrawTileProperties) {
			GV->fntMyriad13->SetColor(0xFF000000);
			GV->fntMyriad13->printf(vPort->GetX() + 11, vPort->GetY() + vPort->GetHeight() - 99, HGETEXT_LEFT, GETL(Lang_Legend));
			GV->fntMyriad13->printf(vPort->GetX() + 61, vPort->GetY() + vPort->GetHeight() - 79, HGETEXT_LEFT, GETL(Lang_PropSolid));
			GV->fntMyriad13->printf(vPort->GetX() + 61, vPort->GetY() + vPort->GetHeight() - 59, HGETEXT_LEFT, GETL(Lang_PropGround));
			GV->fntMyriad13->printf(vPort->GetX() + 61, vPort->GetY() + vPort->GetHeight() - 39, HGETEXT_LEFT, GETL(Lang_PropClimb));
			GV->fntMyriad13->printf(vPort->GetX() + 61, vPort->GetY() + vPort->GetHeight() - 19, HGETEXT_LEFT, GETL(Lang_PropDeath));

			GV->fntMyriad13->SetColor(0xFFFFFFFF);
			GV->fntMyriad13->printf(vPort->GetX() + 10, vPort->GetY() + vPort->GetHeight() - 100, HGETEXT_LEFT, GETL(Lang_Legend));
			GV->fntMyriad13->printf(vPort->GetX() + 60, vPort->GetY() + vPort->GetHeight() - 80, HGETEXT_LEFT, GETL(Lang_PropSolid));
			GV->fntMyriad13->printf(vPort->GetX() + 60, vPort->GetY() + vPort->GetHeight() - 60, HGETEXT_LEFT, GETL(Lang_PropGround));
			GV->fntMyriad13->printf(vPort->GetX() + 60, vPort->GetY() + vPort->GetHeight() - 40, HGETEXT_LEFT, GETL(Lang_PropClimb));
			GV->fntMyriad13->printf(vPort->GetX() + 60, vPort->GetY() + vPort->GetHeight() - 20, HGETEXT_LEFT, GETL(Lang_PropDeath));

			hgeQuad q;
			q.blend = BLEND_DEFAULT;
			q.tex = 0;

			SHR::SetQuad(&q, COLOR_SOLID, vPort->GetX() + 10, vPort->GetY() + vPort->GetHeight() - 80, vPort->GetX() + 50, vPort->GetY() + vPort->GetHeight() - 65);
			hge->Gfx_RenderQuad(&q);
			SHR::SetQuad(&q, COLOR_GROUND, vPort->GetX() + 10, vPort->GetY() + vPort->GetHeight() - 60, vPort->GetX() + 50, vPort->GetY() + vPort->GetHeight() - 45);
			hge->Gfx_RenderQuad(&q);
			SHR::SetQuad(&q, COLOR_CLIMB, vPort->GetX() + 10, vPort->GetY() + vPort->GetHeight() - 40, vPort->GetX() + 50, vPort->GetY() + vPort->GetHeight() - 25);
			hge->Gfx_RenderQuad(&q);
			SHR::SetQuad(&q, COLOR_DEATH, vPort->GetX() + 10, vPort->GetY() + vPort->GetHeight() - 20, vPort->GetX() + 50, vPort->GetY() + vPort->GetHeight() - 5);
			hge->Gfx_RenderQuad(&q);
		}
	}
	...
	if (iActiveTool == EWW_TOOL_ALIGNOBJ) {
		...
		GV->fntMyriad13->printf(vPort->GetX() + 14, yoffset - 61, HGETEXT_LEFT,
			"~l~Shift - %s.", GETL2S("Various", "MoveObj_Info1"));
		GV->fntMyriad13->printf(vPort->GetX() + 15, yoffset - 60, HGETEXT_LEFT,
			"~y~Shift ~w~- %s.", GETL2S("Various", "MoveObj_Info1"));

		GV->fntMyriad13->printf(vPort->GetX() + 14, yoffset - 46, HGETEXT_LEFT,
			"~l~Ctrl - %s.", GETL2S("Various", "MoveObj_Info2"));
		GV->fntMyriad13->printf(vPort->GetX() + 15, yoffset - 45, HGETEXT_LEFT,
			"~y~Ctrl ~w~- %s.", GETL2S("Various", "MoveObj_Info2"));

		GV->fntMyriad13->printf(vPort->GetX() + 14, yoffset - 31, HGETEXT_LEFT, "~l~Esc - %s.", GETL2S("Various", "MoveObj_Info3"));
		GV->fntMyriad13->printf(vPort->GetX() + 15, yoffset - 30, HGETEXT_LEFT, "~y~Esc ~w~- %s.", GETL2S("Various", "MoveObj_Info3"));

	}

	//lower bar
	hge->Gfx_RenderLine(1, hge->System_GetState(HGE_SCREENHEIGHT) - 29, hge->System_GetState(HGE_SCREENWIDTH) - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 29, 0xFF333333);
	hge->Gfx_RenderLine(1, hge->System_GetState(HGE_SCREENHEIGHT) - 3, hge->System_GetState(HGE_SCREENWIDTH) - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 3, 0xFF333333);

	q.v[0].x = 1; q.v[0].y = hge->System_GetState(HGE_SCREENHEIGHT) - 29;
	q.v[1].x = hge->System_GetState(HGE_SCREENWIDTH) - 2; q.v[1].y = hge->System_GetState(HGE_SCREENHEIGHT) - 29;
	q.v[2].x = hge->System_GetState(HGE_SCREENWIDTH) - 2; q.v[2].y = hge->System_GetState(HGE_SCREENHEIGHT) - 4;
	q.v[3].x = 1; q.v[3].y = hge->System_GetState(HGE_SCREENHEIGHT) - 4;
	...
	if (hParser != NULL) {
		...
		GV->fntMyriad13->Render(x, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_LEFT, text);
		x += GV->fntMyriad13->GetStringWidth(text);
		GV->fntMyriad13->SetColor(0xFFeeeeee);
		GV->fntMyriad13->printf(x + 50, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_RIGHT, "%d", iTilesOnScreen);
		x += 55;
		hge->Gfx_RenderLine(x - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 30, x - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 3, GV->colLineDark);
		hge->Gfx_RenderLine(x - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 30, x - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 3, GV->colLineBright);
		...
		x += GV->fntMyriad13->GetStringWidth(text);
		GV->fntMyriad13->SetColor(0xFFeeeeee);
		GV->fntMyriad13->printf(x + 50, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_RIGHT, "%d", iObjectsOnScreen);
		x += 55;
		hge->Gfx_RenderLine(x - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 30, x - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 3, GV->colLineDark);
		hge->Gfx_RenderLine(x - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 30, x - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 3, GV->colLineBright);
		...
		x += GV->fntMyriad13->GetStringWidth(text);
		GV->fntMyriad13->SetColor(0xFFeeeeee);
		GV->fntMyriad13->printf(x + 85, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_RIGHT, "%dx%d", wx, wy);
		x += 90;
		hge->Gfx_RenderLine(x - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 30, x - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 3, GV->colLineDark);
		hge->Gfx_RenderLine(x - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 30, x - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 3, GV->colLineBright);
		...
		x += GV->fntMyriad13->GetStringWidth(text);
		GV->fntMyriad13->SetColor(0xFFeeeeee);
		GV->fntMyriad13->printf(x + 65, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_RIGHT, "%dx%d", tx, ty);
		x += 70;
		hge->Gfx_RenderLine(x - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 30, x - 3, hge->System_GetState(HGE_SCREENHEIGHT) - 3, GV->colLineDark);
		hge->Gfx_RenderLine(x - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 30, x - 2, hge->System_GetState(HGE_SCREENHEIGHT) - 3, GV->colLineBright);

		GV->fntMyriad13->Render(x, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_LEFT, "FPS:");
		GV->fntMyriad13->SetColor(0xFFa1a1a1);
		x += GV->fntMyriad13->GetStringWidth("FPS:");
		GV->fntMyriad13->SetColor(0xFFeeeeee);
		GV->fntMyriad13->printf(x + 30, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_RIGHT, "%d", hge->Timer_GetFPS());
		x += 35;

		GV->fntMyriad13->SetColor(0xFFa1a1a1);
		GV->fntMyriad13->Render(hge->System_GetState(HGE_SCREENWIDTH) - 195, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_RIGHT, GETL2S("Various", "Zoom"));
		GV->fntMyriad13->SetColor(0xFFeeeeee);
		GV->fntMyriad13->printf(hge->System_GetState(HGE_SCREENWIDTH) - 167, hge->System_GetState(HGE_SCREENHEIGHT) - 26, HGETEXT_RIGHT, "%.1fx", fZoom);
	...
	GV->fntMyriad13->printf(840, 25, HGETEXT_LEFT, "~l~Mem: ~y~%s~l~MB", GV->szMemUsage);
	LeaveCriticalSection(&GV->csMemUsage);
#endif

	hge->Gfx_RenderLine(1, 0, 1, hge->System_GetState(HGE_SCREENHEIGHT), GV->colOutline);
	hge->Gfx_RenderLine(hge->System_GetState(HGE_SCREENWIDTH), 0, hge->System_GetState(HGE_SCREENWIDTH), hge->System_GetState(HGE_SCREENHEIGHT), GV->colOutline);

	hge->Gfx_RenderLine(0, hge->System_GetState(HGE_SCREENHEIGHT), hge->System_GetState(HGE_SCREENWIDTH), hge->System_GetState(HGE_SCREENHEIGHT), GV->colOutline);
}
...
bool State::EditingWW::Render()
{
	...
	if (_isOnTop()) {
		...
		if (conMain->getWidgetAt(mx, my) == NULL) {
			if (my >= 3 && my < 24 && (mx > hge->System_GetState(HGE_SCREENWIDTH) - 5 - 20 - 22 && mx < hge->System_GetState(HGE_SCREENWIDTH) - 5 - 20 - 2 ||
				mx > hge->System_GetState(HGE_SCREENWIDTH) - 5 - 20 && mx < hge->System_GetState(HGE_SCREENWIDTH) - 5)) //caption control icons
				bShowHand = 1;
		}
		else {
		...
		q.v[0].x = 0; q.v[0].y = 0;
		q.v[1].x = hge->System_GetState(HGE_SCREENWIDTH); q.v[1].y = 0;
		q.v[2].x = q.v[1].x; q.v[2].y = hge->System_GetState(HGE_SCREENHEIGHT);
		q.v[3].x = 0; q.v[3].y = q.v[2].y;
	...
	if (fade_iAction < 2) {
		...
		q.v[0].x = 0; q.v[0].y = 24;
		q.v[1].x = hge->System_GetState(HGE_SCREENWIDTH); q.v[1].y = 24;
		q.v[2].x = hge->System_GetState(HGE_SCREENWIDTH); q.v[2].y = hge->System_GetState(HGE_SCREENHEIGHT);
		q.v[3].x = 0; q.v[3].y = hge->System_GetState(HGE_SCREENHEIGHT);
		hge->Gfx_RenderQuad(&q);

		GV->sprLogoBig->Render(hge->System_GetState(HGE_SCREENWIDTH) / 2 - 151, hge->System_GetState(HGE_SCREENHEIGHT) / 2 - 38);
		GV->fntMyriad10->Render(hge->System_GetState(HGE_SCREENWIDTH) / 2 - 35, hge->System_GetState(HGE_SCREENHEIGHT) / 2 + 21, HGETEXT_RIGHT, WA_VERSTRING);
...
void State::EditingWW::DrawObjSearch()
{
	...
	for (int i = startindex; i < startindex + 387 / 140 + 2; i++) {
		...
		GV->fntMyriad13->printf(fx + 135, fy + i * 140 - fSliVal, HGETEXT_LEFT, "%s: ~y~%d ~l~(#~y~%i~l~)", GETL2S("ObjectSearch", "ID"), obj->GetParam(WWD::Param_ID), i + 1);
		int yoffset = 20;
		if (strlen(obj->GetName()) > 0) {
			GV->fntMyriad13->printf(fx + 135, fy + i * 140 - fSliVal + yoffset, HGETEXT_LEFT, "%s: ~y~%s~l~", GETL2S("ObjectSearch", "Name"), obj->GetName());
			yoffset += 20;
		}
		if (strlen(obj->GetLogic()) > 0) {
			GV->fntMyriad13->printf(fx + 135, fy + i * 140 - fSliVal + yoffset, HGETEXT_LEFT, "%s: ~y~%s~l~", GETL2S("ObjectSearch", "Logic"), obj->GetLogic());
			yoffset += 20;
		}
		if (strlen(obj->GetImageSet()) > 0) {
			GV->fntMyriad13->printf(fx + 135, fy + i * 140 - fSliVal + yoffset, HGETEXT_LEFT, "%s: ~y~%s~l~", GETL2S("ObjectSearch", "ImageSet"), obj->GetImageSet());
			yoffset += 20;
		}
		if (strlen(obj->GetAnim()) > 0) {
			GV->fntMyriad13->printf(fx + 135, fy + i * 140 - fSliVal + yoffset, HGETEXT_LEFT, "%s: ~y~%s~l~", GETL2S("ObjectSearch", "Anim"), obj->GetAnim());
			yoffset += 20;
		}
		GV->fntMyriad13->printf(fx + 135, fy + i * 140 - fSliVal + yoffset, HGETEXT_LEFT, "%s: ~y~%d~l~x~y~%d~l~", GETL2S("ObjectSearch", "Pos"), obj->GetParam(WWD::Param_LocationX), obj->GetParam(WWD::Param_LocationY));
		yoffset += 20;

		int butx = fx + 135, buty = fy + i * 140 - fSliVal + 97, butw = GV->fntMyriad13->GetStringWidth(GETL2S("ObjectSearch", "GoToObject")) + 8;
		...
		GV->fntMyriad13->Render(butx + butw / 2, buty + 8, HGETEXT_CENTER, GETL2S("ObjectSearch", "GoToObject"));

		hge->Gfx_RenderLine(fx, fy + i * 140 - fSliVal + 135, fx + 424, fy + i * 140 - fSliVal + 135, GV->colLineDark);
		hge->Gfx_RenderLine(fx, fy + i * 140 - fSliVal + 136, fx + 424, fy + i * 140 - fSliVal + 136, GV->colLineBright);
...
int State::EditingWW::RenderObject(WWD::Object * hObj, int x, int y, DWORD col)
{
	...
	spr->SetColor(col);
	spr->SetFlip(GetUserDataFromObj(hObj)->GetFlipX(), GetUserDataFromObj(hObj)->GetFlipY(),/*(list[i]->GetDrawFlags()&WWD::Flag_dr_Mirror), (list[i]->GetDrawFlags()&WWD::Flag_dr_Invert),*/ 1);
	...
	for (int y = 0; y < hPl->GetPlaneHeight(); y++) {
		...
		for (int x = 0; x < hPl->GetPlaneWidth(); x++) {
			...
			if (hPl->GetTile(x, y)->IsInvisible()) {
				...
				q.blend = BLEND_DEFAULT;
				q.tex = 0;
				q.v[0].col = q.v[1].col = q.v[2].col = q.v[3].col = hDataCtrl->GetPalette()->GetColor(hPl->GetFillColor());
				q.v[0].z = q.v[1].z = q.v[2].z = q.v[3].z = 1.0f;
				q.v[0].x = posx; q.v[0].y = posy;
				q.v[1].x = q.v[0].x + hPl->GetTileWidth()*fDrawZoom; q.v[1].y = q.v[0].y;
				q.v[2].x = q.v[1].x; q.v[2].y = q.v[0].y + hPl->GetTileHeight()*fDrawZoom;
				q.v[3].x = q.v[0].x; q.v[3].y = q.v[2].y;
				hge->Gfx_RenderQuad(&q);
				rcount++;
			}
			...
			if (hPlaneData[pl]->bDrawBoundary &&
				((x%hPl->GetPlaneWidth()) == hPl->GetPlaneWidth() - 1 && (hPl->GetFlags() & WWD::Flag_p_YWrapping) != 0 ||
				(x%hPl->GetPlaneWidth()) == hPl->GetPlaneWidth() - 2 && (hPl->GetFlags() & WWD::Flag_p_YWrapping) == 0))
				hge->Gfx_RenderLine(int(posx + hPl->GetTileWidth()*fDrawZoom), 0, int(posx + hPl->GetTileWidth()*fDrawZoom), endplaney, ARGB(255, 255, 0, 255));
			else if (hPlaneData[pl]->bDrawGrid)
				hge->Gfx_RenderLine(int(posx + hPl->GetTileWidth()*fDrawZoom), 0, int(posx + hPl->GetTileWidth()*fDrawZoom), endplaney, ARGB(255, 255, 255, 255));
		}
		if (hPlaneData[pl]->bDrawBoundary &&
			((y%hPl->GetPlaneHeight()) == hPl->GetPlaneHeight() - 1 && (hPl->GetFlags() & WWD::Flag_p_YWrapping) != 0 ||
			(y%hPl->GetPlaneHeight()) == hPl->GetPlaneHeight() - 2 && (hPl->GetFlags() & WWD::Flag_p_YWrapping) == 0))
			hge->Gfx_RenderLine(0, int(posy + hPl->GetTileHeight()*fDrawZoom), endplanex, int(posy + hPl->GetTileHeight()*fDrawZoom), ARGB(255, 255, 0, 255));
		else if (hPlaneData[pl]->bDrawGrid)
			hge->Gfx_RenderLine(0, int(posy + hPl->GetTileHeight()*fDrawZoom), endplanex, int(posy + hPl->GetTileHeight()*fDrawZoom), ARGB(255, 255, 255, 255));
	}
	return rcount;
}
...
void State::EditingWW::DrawSelectFillColor()
{
	...
	if (mx > baseX + 6 && mx < baseX + 196 && my > baseY + 29 && my < baseY + 219 /*&& ( int(my-baseY+10)%12 > 2 || int(my-baseY+10)%12 == 0 )*/) {
		selx = (mx - (baseX + 6)) / 12;
		sely = (my - (baseY + 29)) / 12;
	...
	for (int y = 0; y < 16; y++)
		for (int x = 0; x < 16; x++) {
			SHR::SetQuad(&q, hDataCtrl->GetPalette()->GetColor(y * 16 + x), baseX + 6 + x * 12, baseY + 29 + y * 12, baseX + 6 + x * 12 + 10, baseY + 29 + y * 12 + 10);
			hge->Gfx_RenderQuad(&q);
			if (x == selx && y == sely) {
				hge->Gfx_RenderLine(baseX + 5 + x * 12, baseY + 29 + y * 12, baseX + 6 + x * 12 + 10, baseY + 29 + y * 12, TILE_HIGHLIGHT_COLOR_SOLID);
				hge->Gfx_RenderLine(baseX + 6 + x * 12, baseY + 29 + y * 12, baseX + 6 + x * 12, baseY + 29 + y * 12 + 10, TILE_HIGHLIGHT_COLOR_SOLID);
				hge->Gfx_RenderLine(baseX + 6 + x * 12 + 11, baseY + 28 + y * 12, baseX + 6 + x * 12 + 11, baseY + 30 + y * 12 + 10, TILE_HIGHLIGHT_COLOR_SOLID);
				hge->Gfx_RenderLine(baseX + 5 + x * 12, baseY + 30 + y * 12 + 10, baseX + 6 + x * 12 + 10, baseY + 29 + y * 12 + 10, TILE_HIGHLIGHT_COLOR_SOLID);
			}
			else if (y * 16 + x == GetActivePlane()->GetFillColor()) {
				hge->Gfx_RenderLine(baseX + 5 + x * 12, baseY + 29 + y * 12, baseX + 6 + x * 12 + 10, baseY + 29 + y * 12, TILE_PICK_COLOR_SOLID);
				hge->Gfx_RenderLine(baseX + 6 + x * 12, baseY + 29 + y * 12, baseX + 6 + x * 12, baseY + 29 + y * 12 + 10, TILE_PICK_COLOR_SOLID);
				hge->Gfx_RenderLine(baseX + 6 + x * 12 + 11, baseY + 28 + y * 12, baseX + 6 + x * 12 + 11, baseY + 30 + y * 12 + 10, TILE_PICK_COLOR_SOLID);
				hge->Gfx_RenderLine(baseX + 5 + x * 12, baseY + 30 + y * 12 + 10, baseX + 6 + x * 12 + 10, baseY + 29 + y * 12 + 10, TILE_PICK_COLOR_SOLID);
			}
		}
	int previewcol;
	if (selx == -1 && sely == -1) {
		GV->fntMyriad13->printf(baseX + 5, baseY + 229, HGETEXT_LEFT, "~l~%s: #~y~%d", GETL(Lang_ActualFillColor), GetActivePlane()->GetFillColor());
		previewcol = GetActivePlane()->GetFillColor();
	}
	else {
		GV->fntMyriad13->printf(baseX + 5, baseY + 229, HGETEXT_LEFT, "~l~%s: #~y~%d", GETL(Lang_SelectedFillColor), sely * 16 + selx);
...
{
	if (hPl == GetActivePlane() && iMode == EWW_MODE_TILE && (bObjDragSelection || iTileSelectX1 != -1 && iTileSelectY1 != -1)) {
		int x1, y1, x2, y2;
...
void State::EditingWW::ViewportOverlay()
{
	...
	if (bShowConsole) {
		for (int i = 1; i < (int(vPort->GetHeight() / 15) + 1); i++) {
			...
			GV->fntSystem17->Render(6, vPort->GetY() + vPort->GetHeight() - i * 15 + 1, HGETEXT_LEFT, line);
			GV->fntSystem17->SetColor(0xFFFFFFFF);
			GV->fntSystem17->Render(5, vPort->GetY() + vPort->GetHeight() - i * 15, HGETEXT_LEFT, line);
		}
	}
#endif
}
...
void State::EditingWW::RenderCloudTip(int x, int y, int w, int h, int ax, int ay)
{
	...
	tr.v[0].x = x + 25; tr.v[0].y = y + h;
	tr.v[1].x = x + 45; tr.v[1].y = y + h;
	tr.v[2].x = ax; tr.v[2].y = ay;
...
void State::EditingWW::RenderTileClipboardPreview()
{
	...
	if (MDI->GetActiveDoc()->hTileClipboard == NULL) {
		...
		RenderCloudTip(10, hge->System_GetState(HGE_SCREENHEIGHT) - 70, len + 20, 20, butMicroTileCB->getX() + 9, butMicroTileCB->getY() - 3);
		GV->fntMyriad13->Render(20, hge->System_GetState(HGE_SCREENHEIGHT) - 70, HGETEXT_LEFT, GETL2S("ClipboardPreview", "TileClipboardEmpty"));
		return;
	}
	...
	RenderCloudTip(px - 10, py - 10, tilew*cbw + 20, tileh*cbh + 20, butMicroTileCB->getX() + 9, butMicroTileCB->getY() - 3);
	for (int y = 0; y < cbh; y++)
		for (int x = 0; x < cbw; x++) {
			if (MDI->GetActiveDoc()->hTileClipboard[x][y].IsInvisible()) continue;
				...
				hgeSprite * spr = hTileset->GetTile(MDI->GetActiveDoc()->hTileClipboardImageSet, MDI->GetActiveDoc()->hTileClipboard[x][y].GetID())->GetImage();
				spr->SetColor(0xFFFFFFFF);
				spr->SetFlip(0, 0);
				spr->SetHotSpot(0, 0);
...
void State::EditingWW::RenderObjectClipboardPreview()
{
	...
	if (vObjectClipboard.size() == 0) {
		...
		RenderCloudTip(10, hge->System_GetState(HGE_SCREENHEIGHT) - 70, len + 20, 20, butMicroObjectCB->getX() + 9, butMicroObjectCB->getY() - 3);
		GV->fntMyriad13->Render(20, hge->System_GetState(HGE_SCREENHEIGHT) - 70, HGETEXT_LEFT, GETL2S("ClipboardPreview", "ObjectClipboardEmpty"));
		return;
	}
	int sizeX = 0, sizeY = 0;
	WWD::Object * horBase = NULL, *verBase = NULL;
	{
		...
		for (int i = 0; i < vObjectClipboard.size(); i++) {
			...
			maxx = maxx == -1 ? vObjectClipboard[i]->GetParam(WWD::Param_LocationX) + spr->GetWidth() / 2 : std::max(maxx, int(vObjectClipboard[i]->GetParam(WWD::Param_LocationX) + spr->GetWidth() / 2));
			maxy = maxy == -1 ? vObjectClipboard[i]->GetParam(WWD::Param_LocationY) + spr->GetHeight() / 2 : std::max(maxy, int(vObjectClipboard[i]->GetParam(WWD::Param_LocationY) + spr->GetHeight() / 2));
		}
		sizeX = maxx - minx;
		sizeY = maxy - miny;
	}
	...
	for (int i = 0; i < vObjectClipboard.size(); i++) {
		hgeSprite * spr = SprBank->GetObjectSprite(vObjectClipboard[i]);
		int offx = vObjectClipboard[i]->GetParam(WWD::Param_LocationX) - baseX, offy = vObjectClipboard[i]->GetParam(WWD::Param_LocationY) - baseY;
		spr->SetColor(0xFFFFFFFF);
		spr->SetFlip(GetUserDataFromObj(vObjectClipboard[i])->GetFlipX(), GetUserDataFromObj(vObjectClipboard[i])->GetFlipY());
...
void State::EditingWW::DrawCrashRetrieve()
{
	...
	GV->fntMyriad13->SetColor(0xFFa1a1a1);
	GV->fntMyriad13->Render(dx + 5, dy + 3, HGETEXT_LEFT, GETL2S("WinCrashRetrieve", "Label"));
	int c = 0;
	for (int i = 0; i < 10; i++) {
		...
		GV->fntMyriad13->Render(dx + 20, dy + 25 + 25 * i, HGETEXT_LEFT, szCrashRetrieve[i]);
		c++;
	}
	if (szCrashRetrieveMore != NULL)
		GV->fntMyriad13->Render(dx + 5, dy + 25 + 25 * c, HGETEXT_LEFT, szCrashRetrieveMore);
}
...
void State::EditingWW::DrawTilePicker()
{
	...
	hge->Gfx_RenderLine(dx, dy + iTilePickerOffUp + 13, dx + winTilePicker->getWidth(), dy + iTilePickerOffUp + 13, GV->colLineDark);
	hge->Gfx_RenderLine(dx, dy + iTilePickerOffUp + 14, dx + winTilePicker->getWidth(), dy + iTilePickerOffUp + 14, GV->colLineBright);
	...
	if (set) {
		...
		if (iActiveTool == EWW_TOOL_BRUSH) {
			...
			if (set->GetBrushesCount() == 0) {
				GV->fntMyriad13->SetColor(0xFF000000);
				GV->fntMyriad13->printf(itpx + 115, itpy + slitpiPicker->getHeight() / 2, HGETEXT_CENTER, GETL(Lang_NoBrushes));
			}
			for (int i = 0; i < set->GetBrushesCount() && i < (loopst + loopen); i++) {
				...
				if (hasmouse &&
					(mx > drx && my > dry && mx < drx + 48 && my < dry + 48 ||
						mx > drx + 55 && mx < drx + 55 + GV->fntMyriad13->GetStringWidth(brush->GetName()) && my > dry + 14 && my < dry + 34)
				...
				GV->fntMyriad13->Render(drx + 55, dry + 14, HGETEXT_LEFT, brush->GetName());
			}
			...
			for (int i = -3 + loopst; i < set->GetTilesCount() && i < (-2 + loopst + loopen); i++) {
				...
				if (i == -3) {
					...
					q.tex = 0;
					q.blend = BLEND_DEFAULT;
					SHR::SetQuad(&q, hDataCtrl->GetPalette()->GetColor(GetActivePlane()->GetFillColor()), drx, dry, drx + 48, dry + 48);
					hge->Gfx_RenderQuad(&q);
				}
				else {
					...
					if (cbtpiShowTileID->isSelected()) {
						GV->fntMyriad13->SetColor(0xFF000000);
						GV->fntMyriad13->printf(drx + 24 + 1, dry + 1 + 32, HGETEXT_CENTER, "%d", tile->GetID());
						GV->fntMyriad13->SetColor(0xFFFFFFFF);
						GV->fntMyriad13->printf(drx + 24, dry + 32, HGETEXT_CENTER, "%d", tile->GetID());
					}
				}
				hge->Gfx_RenderLine(drx - 2, dry - 1, drx + 50, dry - 1, GV->colLineDark);
				hge->Gfx_RenderLine(drx - 1, dry + 50, drx + 50, dry + 50, GV->colLineDark);
				...
				if (mx > drx && my > dry && mx < drx + 48 && my < dry + 48 && hasmouse && my > itpy && my < itpy + slitpiPicker->getHeight()) {
					colBorder = TILE_HIGHLIGHT_COLOR;
					bShowHand = 1;
					if (hge->Input_KeyDown(HGEK_LBUTTON)) {
				...
				else if (i < 0 && i - 1 == iTilePicked || i >= 0 && i == iTilePicked || iTilePicked == EWW_TILE_PIPETTE && i == -3) {
					colBorder = TILE_PICK_COLOR;
				}
				hge->Gfx_RenderLine(drx - 1, dry, drx + 49, dry, colBorder);
				hge->Gfx_RenderLine(drx, dry + 49, drx + 49, dry + 49, colBorder);
		...
		GV->sprShadeBar->RenderStretch(itpx, itpy + slitpiPicker->getHeight() - 6, itpx + 230, itpy + slitpiPicker->getHeight());
