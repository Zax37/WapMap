...
void State::EditingWW::HandleHotkeys()
{
    ...
    if( bFocus && hge->Input_GetKeyState(HGEK_CTRL) && hge->Input_KeyDown(HGEK_A) ){
     if( iMode == EWW_MODE_TILE ){
      ...
      vObjectsPicked = hPlaneData[GetActivePlaneID()]->ObjectData.hQuadTree->GetObjectsByArea(0, 0, GetActivePlane()->GetPlaneWidthPx(), GetActivePlane()->GetPlaneHeightPx());
     }
    ...
    }else if( iMode == EWW_MODE_OBJECT && hge->Input_GetKeyState(HGEK_CTRL) && hge->Input_KeyDown(HGEK_F) && hmbObject->butIconSearchObject->isEnabled() ){
     hmbObject->butIconSearchObject->simulatePress();
    }else if( bFocus && hge->Input_GetKeyState(HGEK_CTRL) && hge->Input_KeyDown(HGEK_X) && vObjectsPicked.size() != 0 && iMode == EWW_MODE_OBJECT ){
     objContext->EmulateClickID(OBJMENU_CUT);
    }else if( bFocus && hge->Input_GetKeyState(HGEK_CTRL) && hge->Input_KeyDown(HGEK_C) && vObjectsPicked.size() != 0 && iMode == EWW_MODE_OBJECT ){
     ...
     tilContext->EmulateClickID(TILMENU_CUT);
              ...
              (hge->Input_GetKeyState(HGEK_CTRL) && hge->Input_KeyDown(HGEK_V) || bFocus && hge->Input_GetKeyState(HGEK_TAB) && hge->Input_KeyDown(HGEK_LBUTTON)) ){
    ...
    }else if( bFocus && hge->Input_GetKeyState(HGEK_CTRL) && hge->Input_KeyDown(HGEK_V) && vObjectClipboard.size() != 0 && iMode == EWW_MODE_OBJECT ){
     float mx, my;
     hge->Input_GetMousePos(&mx, &my);
     objContext->setX(mx);
     objContext->setY(my);
    ...
    }else if( bFocus && iMode == EWW_MODE_TILE && (iActiveTool == EWW_TOOL_BRUSH || iActiveTool == EWW_TOOL_PENCIL || iActiveTool == EWW_TOOL_FILL) ){
     ...
     if( hge->Input_KeyDown(HGEK_ADD) ){
      if( (iActiveTool == EWW_TOOL_PENCIL && iTilePicked+1 < hTileset->GetSet(GetActivePlane()->GetImageSet(0))->GetTilesCount()) ||
          (iActiveTool == EWW_TOOL_BRUSH  && iTilePicked+1 < hTileset->GetSet(GetActivePlane()->GetImageSet(0))->GetBrushesCount()) ){
       iTilePicked++;
       vPort->MarkToRedraw(1);
      }
    ...
    }else if( iMode == EWW_MODE_OBJECT && vObjectsPicked.size() != 0 && conMain->getWidgetAt(mx, my) == vPort->GetWidget() &&
             (hge->Input_GetKeyState(HGEK_CTRL) || hge->Input_GetKeyState(HGEK_SHIFT) || hge->Input_GetKeyState(HGEK_ALT)) &&
             (hge->Input_KeyDown(HGEK_LEFT) || hge->Input_KeyDown(HGEK_RIGHT) || hge->Input_KeyDown(HGEK_UP) || hge->Input_KeyDown(HGEK_DOWN)) ){
     ...
     for(int i=0;i<vObjectsPicked.size();i++){
      ...
      posx += moveresx;
      posy += moveresy;
      if( posx < 0 || posy < 0 || posx >= GetActivePlane()->GetPlaneWidthPx() || posy >= GetActivePlane()->GetPlaneHeightPx() )
       continue;
     ...
     vPort->MarkToRedraw(1);
    }else if( hge->Input_KeyDown(HGEK_M) && iMode == EWW_MODE_OBJECT && vObjectsPicked.size() != 0 && vPort->GetWidget()->isMouseOver() ){
     for(int i=0;i<vObjectsPicked.size();i++){
      if( vObjectsPicked[i]->GetDrawFlags() & WWD::Flag_dr_Mirror )
       vObjectsPicked[i]->SetDrawFlags((WWD::OBJ_DRAW_FLAGS)(int(vObjectsPicked[i]->GetDrawFlags()) - WWD::Flag_dr_Mirror));
      else
       vObjectsPicked[i]->SetDrawFlags((WWD::OBJ_DRAW_FLAGS)(int(vObjectsPicked[i]->GetDrawFlags()) + WWD::Flag_dr_Mirror));
     }
     MarkUnsaved();
    ...
    /*}else if( hge->Input_GetKeyState(HGEK_SHIFT) && hge->Input_KeyDown(HGEK_TAB) && iMode == EWW_MODE_OBJECT ){
     ...
     fCamX = GetActivePlane()->GetObjectByIterator(iObjectPicked)->GetParam(WWD::Param_LocationX)-(vPort->GetWidth()/2/fZoom);
     fCamY = GetActivePlane()->GetObjectByIterator(iObjectPicked)->GetParam(WWD::Param_LocationY)-(vPort->GetHeight()/2/fZoom);*/
    }else if( bFocus && hge->Input_GetKeyState(HGEK_CTRL) && hge->Input_KeyDown(HGEK_D) && iMode == EWW_MODE_OBJECT && vObjectsPicked.size() != 0 ){
