...
void State::EditingWW::NewMap_Open()
{
	...
	conMain->add(NewMap_data->winNewMap, hge->System_GetState(HGE_SCREENWIDTH) / 2 - 325, hge->System_GetState(HGE_SCREENHEIGHT) / 2 - 292);
...
void State::EditingWW::NewMap_Think()
{
	...
	if (mx > wx + 27 && mx < wx + 627 && my > wy + 101 && my < wy + 491 && bMouseOn)
		NewMap_data->sliBaseLvls->setValue(NewMap_data->sliBaseLvls->getValue() + NewMap_data->sliBaseLvls->getStepLength()*hge->Input_GetMouseWheel());
#ifdef WM_EXT_TILESETS
...
{
	int plw = atoi(NewMap_data->tfPlaneWidth->getText().c_str()), plh = atoi(NewMap_data->tfPlaneHeight->getText().c_str());
	if (plw < 50 || plh < 50) {
		GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("NewMap", "InvalidPlaneSize"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
		return;
	}
	//repo.PrintStructure();
	void * ptr;
	...
	GV->StateMgr->Push(new State::LoadMap(ptr, size, NewMap_data->cbAddBonusPlanes->isSelected(), plw, plh, name, author));
	NewMap_data->bKill = 1;
}
