...
ImageImport::ImageImport(ImageImportType type, void * hDest)
{
    ...
    myWin = new SHR::Win(&GV->gcnParts, GETL2S("Win_ImageImport", iImportType == ImageImportTile ? "WinCaption" : "WinCaption2"));
    //int height = hge->System_GetState(HGE_SCREENHEIGHT)-200;
    myWin->setDimension(gcn::Rectangle(hge->System_GetState(HGE_SCREENWIDTH)/2-300, hge->System_GetState(HGE_SCREENHEIGHT)/2-250-yOffset/2, 600, 565+yOffset));
    myWin->setMovable(0);
    myWin->setClose(1);
    ...
    if( !hDest ){
     labDestName = new SHR::Lab(GETL2S("Win_ImageImport", iImportType == ImageImportTile ? "TilesetName" : "ImagesetName"));
     ...
     labWarnDestName = new SHR::Lab(GETL2S("Win_ImageImport", iImportType == ImageImportTile ? "WarnSpecifyTileset" : "WarnSpecifyImageset"));
...
void ImageImport::action(const gcn::ActionEvent &actionEvent)
{
    if( actionEvent.getSource() == myWin || actionEvent.getSource() == butCancel ){
     ...
     if( vImages.size() > 0 ){
      ...
      if( iImportType == ImageImportTile ){
       ...
       if( hDestinationTS != 0 ){
        ...
        DIR * d = opendir(strSubPath.c_str());
        if( !d )
       ...
       while( 1 ){
        ...
        DIR * d = opendir(strAbsPath.c_str());
        if( !d )
     ...
     if( tfDestName->getText().length() == 0 ){
      labWarnDestName->setCaption(GETL2S("Win_ImageImport", iImportType == ImageImportTile ? "WarnSpecifyTileset" : "WarnSpecifyImageset"));
    ...
    for(size_t i=0;i<vImages.size();i++){
     if( actionEvent.getSource() == vImages[i].butRemove ){
      ...
      if( vImages[i].tfID->getText().length() == 0 ){
       ...
       vImages[i].labWarnID->setVisible(1);
       vImages[i].labWarnID->setCaption(GETL2S("Win_ImageImport", iImportType == ImageImportTile ? "WarnUsedByCustomTile" : "WarnUsedByCustomImage"));
       vImages[i].labWarnID->setColor(0xa13a3a);
       vImages[i].labWarnID->adjustSize();
       if( !vImages[i].bBadFlag ){
        vImages[i].bBadFlag = 1;
       ...
       vImages[i].labWarnID->setVisible(1);
       vImages[i].labWarnID->setCaption(GETL2S("Win_ImageImport", iImportType == ImageImportTile ? "InfoTileReplace" : "InfoImageReplace"));
    ...
    if( piCode == 0 ){
     ...
     if( vImages.empty() ){
      ...
      for(size_t i=first;i<vImages.size()&&i<(first+amount);i++){
       ...
       hge->Gfx_RenderLine(drx, yPos+iListElementHeight-2, drx+drw, yPos+iListElementHeight-2, GV->colLineBright);
       hge->Gfx_RenderLine(drx, yPos+iListElementHeight-1, drx+drw, yPos+iListElementHeight-1, GV->colLineDark);
      }
     }
...
void ImageImport::FixPosition(size_t imgID)
{
    ...
    vImages[imgID].butRemove->setPosition(saFileList->getChildrenArea().width-5-32, yPos+iListElementHeight/2-16);
