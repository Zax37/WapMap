...
void State::EditingWW::Init()
{
    ...
    vPort = new Viewport(this, 0, 50+24, hge->System_GetState(HGE_SCREENWIDTH)-17, hge->System_GetState(HGE_SCREENHEIGHT)-31-25-24-25-11);
    ...
    SetIconBarVisible(0);
     ...
     conResizeRight->setDimension(gcn::Rectangle(vPort->GetX()+vPort->GetWidth()-40, vPort->GetY()+40, 40, vPort->GetHeight()-80));
     ...
     conResizeDown->setDimension(gcn::Rectangle(vPort->GetX(), vPort->GetY()+vPort->GetHeight()-40, vPort->GetWidth(), 40));
     ...
     if( GV->szClawPath != 0 )
      hNativeController->SetPath(GV->szClawPath);
     ...
     conMain->add(winSearchObj, hge->System_GetState(HGE_SCREENWIDTH)/2-225, hge->System_GetState(HGE_SCREENHEIGHT)/2-257);
     ...
     conMain->add(winSpacing, hge->System_GetState(HGE_SCREENWIDTH)/2-100, hge->System_GetState(HGE_SCREENHEIGHT)/2-50);
     ...
     winDB->setVisible(0);
     winDB->setClose(1);
     conMain->add(winDB, hge->System_GetState(HGE_SCREENWIDTH)/2-400, hge->System_GetState(HGE_SCREENHEIGHT)/2-300);
     ...
     if( hParser != NULL ){
      char tmp[128];
      sprintf(tmp, GETL(Lang_MapShotDimensions), int(GetActivePlane()->GetPlaneWidthPx()/4.0f), int(GetActivePlane()->GetPlaneHeightPx()/4.0f));
     ...
     winpmMain->setDimension(gcn::Rectangle(0, 0, 225, 275));
     winpmMain->setVisible(0);
     winpmMain->setClose(1);
     conMain->add(winpmMain, hge->System_GetState(HGE_SCREENWIDTH)/2-112, hge->System_GetState(HGE_SCREENHEIGHT)/2-162);
     ...
     //conpmPlanes->add(lbpmPlanes, 0, 0);
     ...
     sapmPlanes->setDimension(gcn::Rectangle(0, 0, 210, 227));
     ...
     winpmMain->add(tfpmPlaneSizeX, 301, 127);
     ...
     winpmMain->add(tfpmPlaneSizeY, 301, 157);

     ipmAnchor = 5;
     ipmSizeX = ipmSizeY = -1;

     butpmResUL = MakeButton(525-96-35, 120, Icon_UpLeft, winpmMain, 1, 1);
     butpmResU = MakeButton(525-64-35, 120, Icon_Up, winpmMain, 1, 1);
     butpmResUR = MakeButton(525-32-35, 120, Icon_UpRight, winpmMain, 1, 1);

     butpmResL = MakeButton(525-96-35, 120+32, Icon_Left, winpmMain, 1, 1);
     butpmResC = MakeButton(525-64-35, 120+32, Icon_Anchor, winpmMain, 1, 1);
     butpmResR = MakeButton(525-32-35, 120+32, Icon_Right, winpmMain, 1, 1);

     butpmResDL = MakeButton(525-96-35, 120+64, Icon_DownLeft, winpmMain, 1, 1);
     butpmResD = MakeButton(525-64-35, 120+64, Icon_Down, winpmMain, 1, 1);
     butpmResDR = MakeButton(525-32-35, 120+64, Icon_DownRight, winpmMain, 1, 1);
     ...
     winpmMain->add(butpmSave, 258-35, 225);
     ...
     winpmMain->add(butpmDelete, 358-25, 225);
     ...
     winWorld->setDimension(gcn::Rectangle(0, 0, 800, 280));
     winWorld->setVisible(0);
     winWorld->setClose(1);
     ...
     winWorld->add(butwpSave, 410, 210);
     winWorld->add(butwpCancel, 520, 210);
     ...
     conMain->add(winDuplicate, hge->System_GetState(HGE_SCREENWIDTH)/2-105, hge->System_GetState(HGE_SCREENHEIGHT)/2-100);
     ...
     conMain->add(winWelcome, hge->System_GetState(HGE_SCREENWIDTH)/2-300, hge->System_GetState(HGE_SCREENHEIGHT)/2-150);
     ...
     if( hMruList->IsValid() && hMruList->GetFilesCount() > 0 || conCrashRetrieve ){
      ...
      winWelcome->add(contenertoadd, winWelcome->getWidth()/2-contenertoadd->getWidth()/2, winWelcome->getHeight()-contenertoadd->getHeight());
     }
    ...
    objmAlignContext->AddElement(OBJMENU_ALIGN_HOR, GETL2S("Various", "ContextAlign_Hor"), GV->sprIcons16[Icon16_FlipX]);
    objmAlignContext->AddElement(OBJMENU_ALIGN_VERT, GETL2S("Various", "ContextAlign_Vert"), GV->sprIcons16[Icon16_FlipY]);
    ...
    objmSpaceContext->AddElement(OBJMENU_SPACE_VERT, GETL2S("Various", "ContextSpaceVer"), GV->sprIcons16[Icon16_FlipY]);
    ...
    conmodPaste->AddElement(OBJMENU_TESTFROMHERE, GETL2S("Various", "ContextTestFromHere"), GV->sprIcons16[Icon16_Play]);
    ...
    conmodAtEmpty->AddElement(OBJMENU_TESTFROMHERE, GETL2S("Various", "ContextTestFromHere"), GV->sprIcons16[Icon16_Play]);
    ...
    objZCoordContext->AddElement(OBJMENU_ZC_INC3, GETL2S("Various", "ContextZ_Increase3"), GV->sprIcons16[Icon16_AddTriple]);
    objZCoordContext->AddElement(OBJMENU_ZC_INC2, GETL2S("Various", "ContextZ_Increase2"), GV->sprIcons16[Icon16_AddDouble]);
    ...
    objZCoordContext->AddElement(OBJMENU_ZC_DEC2, GETL2S("Various", "ContextZ_Decrease2"), GV->sprIcons16[Icon16_SubDouble]);
    objZCoordContext->AddElement(OBJMENU_ZC_DEC3, GETL2S("Various", "ContextZ_Decrease3"), GV->sprIcons16[Icon16_SubTriple]);
    objZCoordContext->AddElement(OBJMENU_ZC_BACK, GETL2S("Various", "ContextZ_Back"), GV->sprIcons16[Icon16_PlaneBack]);
    objZCoordContext->AddElement(OBJMENU_ZC_ACTION, GETL2S("Various", "ContextZ_Action"), GV->sprIcons16[Icon16_PlaneAction]);
    objZCoordContext->AddElement(OBJMENU_ZC_FRONT, GETL2S("Various", "ContextZ_Front"), GV->sprIcons16[Icon16_PlaneFront]);
    ...
    advcon_Container->AddElement(OBJMENU_ADV_CONTAINER_RAND, GETL2S("EditObj_Statue", "Context_Randomize"), GV->sprIcons16[Icon16_Treasure]);
    ...
    /*if( GV->szClawPath == 0 ){
     winOptions->setVisible(1);
     conMain->moveToTop(winOptions);
    }*/
}
...
bool State::EditingWW::Think()
{
    ...
    if( hParser == NULL || iMode != EWW_MODE_TILE ){
     ...
     if( fCamX > maxx && !(GetActivePlane()->GetFlags() & WWD::Flag_p_XWrapping) ){
      conResizeRight->setDimension(gcn::Rectangle(vPort->GetX()+vPort->GetWidth()-(fCamX-maxx), vPort->GetY()+40, fCamX-maxx, vPort->GetHeight()-80));
     ...
     if( fCamY > maxy && !(GetActivePlane()->GetFlags() & WWD::Flag_p_YWrapping) ){
      conResizeDown->setDimension(gcn::Rectangle(vPort->GetX(), vPort->GetY()+vPort->GetHeight()-(fCamY-maxy), vPort->GetWidth(), fCamY-maxy));
    ...
    if( bDragWindow ){
     if( !hge->Input_GetKeyState(HGEK_LBUTTON) ){
      ...
      float dmx, dmy;
      RECT r;
      GetWindowRect(hge->System_GetState(HGE_HWND), &r);
      dmx = mouse.x-r.left;
      dmy = mouse.y-r.top;

      SetWindowPos(hge->System_GetState(HGE_HWND), NULL, r.left-(iWindowDragX-dmx), r.top-(iWindowDragY-dmy), 0, 0, SWP_NOSIZE|SWP_NOZORDER);
      hge->Input_SetMousePos(iWindowDragX, iWindowDragY);
     }
    }
    ...
    if( hge->Input_KeyDown(HGEK_LBUTTON) && conMain->getWidgetAt(mx,my) == NULL && my >= 2 && my < 24 ){
     ...
     if( mx > hge->System_GetState(HGE_SCREENWIDTH)-5-20-22 && mx < hge->System_GetState(HGE_SCREENWIDTH)-5-22 ) //minimize
      ShowWindow(hge->System_GetState(HGE_HWND), SW_MINIMIZE);
     else if( mx > hge->System_GetState(HGE_SCREENWIDTH)-5-20 && mx < hge->System_GetState(HGE_SCREENWIDTH)-5 ) //close
      if( PromptExit() ){
       bExit = 1;
       return 1;
      }
    ...
    //fCamX = sliHor->getValue();
    //fCamY = GetActivePlane()->GetPlaneHeightPx()-sliVer->getValue();
    GV->Console->Think();
    ...
    /*for(int i=0;i<vObjectWins.size();i++){
     if( vObjectWins[i]->Kill() ){
      delete vObjectWins[i];
      vObjectWins.erase(vObjectWins.begin()+i);
      i--;
     }
    }*/
    ...
    if( GV->editState->MDI->GetActiveDoc()->vGuides.size() > 0 && bShowGuideLines ){
     ...
     if( conMain->getWidgetAt(mx, my) == vPort->GetWidget() ){
      for(size_t i=0;i<GV->editState->MDI->GetActiveDoc()->vGuides.size();i++){
       ...
       if( gl.bOrient == GUIDE_HORIZONTAL && (scrpos < vPort->GetY() || scrpos > vPort->GetY()+vPort->GetHeight()) ||
           gl.bOrient == GUIDE_VERTICAL   && (scrpos < vPort->GetX() || scrpos > vPort->GetX()+vPort->GetWidth()) )
        continue;
       if( (gl.bOrient == GUIDE_HORIZONTAL && (my > scrpos-5 && my < scrpos+5) ||
           gl.bOrient == GUIDE_VERTICAL   && (mx > scrpos-5 && mx < scrpos+5)) && iActiveTool != EWW_TOOL_MOVEOBJECT && iActiveTool != EWW_TOOL_EDITOBJ ){
        bMouseConsumed = 1;
        bShowHand = 1;
     ...
     if( iManipulatedGuide != -1 ){
      ...
      if( conMain->getWidgetAt(mx,my) == vPort->GetWidget() ){
       ...
       if( hge->Input_GetKeyState(HGEK_LBUTTON) ){
        ...
        GV->editState->MDI->GetActiveDoc()->vGuides.erase(GV->editState->MDI->GetActiveDoc()->vGuides.begin()+iManipulatedGuide);
        iManipulatedGuide = -1;
       }
      }
      bMouseConsumed = 1;
    ...
    if( iActiveTool == EWW_TOOL_MEASURE && !bMouseConsumed ){
     if( hge->Input_KeyDown(HGEK_LBUTTON) ){
      int scrx = mx, scry = my;
      if( hge->Input_GetKeyState(HGEK_SHIFT) && m_vMeasurePoints.size() != 0 ){
      ...
      m_vMeasurePoints.push_back(std::make_pair(Scr2WrdX(GetActivePlane(), scrx), Scr2WrdY(GetActivePlane(), scry)));
     }
    }

    bool scrollcatched = 0;
    bool hotkeyused = hge->Input_GetKeyState(HGEK_SHIFT) || hge->Input_GetKeyState(HGEK_CTRL) || hge->Input_GetKeyState(HGEK_ALT);
    if( mx > vPort->GetX() && mx < vPort->GetX()+vPort->GetWidth() && my > vPort->GetY() && my < vPort->GetY()+vPort->GetHeight()&& !bMouseConsumed ){
    ...
    if( (cScrollOrientation == 2 && !hotkeyused || hge->Input_GetKeyState(HGEK_ALT)) && !scrollcatched ){
     ...
     if( mx > vPort->GetX() && mx < vPort->GetX()+vPort->GetWidth() && my > vPort->GetY() && my < vPort->GetY()+vPort->GetHeight()&& !bMouseConsumed && hge->Input_KeyDown(HGEK_RBUTTON) ){
      float mx, my;
      hge->Input_GetMousePos(&mx, &my);
      bDragDropScroll = 1;
      fDragLastMx = mx;
    ...
    if( mx > hge->System_GetState(HGE_SCREENWIDTH)-11 &&
     ...
     switch( cScrollOrientation ){
      case 0: SetHint(GETL(Lang_ScrollScrollsVerticaly)); break;
      case 1: SetHint(GETL(Lang_ScrollScrollsHorizontaly)); break;
      case 2: SetHint(GETL(Lang_ScrollZooms)); break;
     }
    }
    //SHORTCUTS
    HandleHotkeys();
    ...
    if( !bMouseConsumed && (vObjectsPicked.size() == 0 ||
                            !hge->Input_GetKeyState(HGEK_CTRL) && !hge->Input_GetKeyState(HGEK_SHIFT) && !hge->Input_GetKeyState(HGEK_ALT)) ){
     if( fCameraMoveTimer == 0 || fCameraMoveTimer > 0.1 ){
      ...
      if( vPort->GetWidget()->isFocused() && bWindowFocused ){
       if( hge->Input_GetKeyState(HGEK_UP) ){
        fCamY -= (int(fCamY)%GetActivePlane()->GetTileHeight() != 0 ? int(fCamY)%GetActivePlane()->GetTileHeight() : GetActivePlane()->GetTileHeight());
       }else if( hge->Input_GetKeyState(HGEK_DOWN) ){
        fCamY += (int(fCamY)%GetActivePlane()->GetTileHeight() != 0 ? int(fCamY)%GetActivePlane()->GetTileHeight() : GetActivePlane()->GetTileHeight());
       }
       if( hge->Input_GetKeyState(HGEK_LEFT) ){
        fCamX -= (int(fCamX)%GetActivePlane()->GetTileWidth() != 0 ? int(fCamX)%GetActivePlane()->GetTileWidth() : GetActivePlane()->GetTileWidth());
    ...
    if( !(GetActivePlane()->GetFlags() & WWD::Flag_p_XWrapping) )
     if( fCamX > GetActivePlane()->GetPlaneWidthPx()-vPort->GetWidth()/fZoom+xresize )
      fCamX = std::max(GetActivePlane()->GetPlaneWidthPx()-vPort->GetWidth()/fZoom+xresize, float(-xresize));

    if( !(GetActivePlane()->GetFlags() & WWD::Flag_p_YWrapping) )
     if( fCamY > GetActivePlane()->GetPlaneHeightPx()-vPort->GetHeight()/fZoom+yresize )
      fCamY = std::max(GetActivePlane()->GetPlaneHeightPx()-vPort->GetHeight()/fZoom+yresize, float(-yresize));
    ...
    if( ((returnCode*)iReturnCode)->Type == RC_LoadMap ){
     ...
     if( ((returnCode*)iReturnCode)->Ptr == 1 ){
      ...
      if( cbmsOpenFileDirectory->isSelected() ){
       char command[strlen(tfmsSaveAs->getText().c_str())+32];
...
void State::EditingWW::OpenTool(int iTool)
{
    if( iTool == EWW_TOOL_PENCIL || iTool == EWW_TOOL_FILL){
     ...
     butTilePickerPipette->setVisible(1);
     sliTilePicker->setVisible(1);
     butTilePickerRight->setVisible(1);
     butTilePickerLeft->setVisible(1);
     butTilePickerType->setVisible(1);
     sliTilePicker->setValue(0);
     if( butTilePickerType->getIcon() == GV->sprIcons[Icon_Row] )
      sliTilePicker->setScaleEnd(std::max(0, hTileset->GetSet(GetActivePlane()->GetImageSet(0))->GetTilesCount()*80-hge->System_GetState(HGE_SCREENWIDTH)+91));
     else if( butTilePickerType->getIcon() == GV->sprIcons[Icon_Grid] )
      sliTilePicker->setScaleEnd(std::max(0, (hTileset->GetSet(GetActivePlane()->GetImageSet(0))->GetTilesCount()/3+1)*32-hge->System_GetState(HGE_SCREENWIDTH)+91));
     return;*/
...
void State::EditingWW::CloseTool(int iTool)
{
    if( iActiveTool == EWW_TOOL_PENCIL || iActiveTool == EWW_TOOL_FILL ){
     ...
     if( iTilePicked != -1 && MDI->GetActiveDoc() != NULL )
      hTileset->GetSet(GetActivePlane()->GetImageSet(0))->GetBrushByIterator(iTilePicked)->RemoveSettingsFromContainer(winTilePicker);
     vTileGhosting.clear();
     return;
...
void State::EditingWW::UpdateScrollBars()
{
    ...
    sliHor->setScaleEnd(std::max(GetActivePlane()->GetPlaneWidthPx()-vPort->GetWidth()/fZoom+40*(condx), -40.0f));
    sliVer->setScaleEnd(std::max(GetActivePlane()->GetPlaneHeightPx()-vPort->GetHeight()/fZoom+40*(condy), -40.0f));
...
void State::EditingWW::SyncLogicBrowser()
{
    ...
    bool validselect = (lbbrlLogicList->getSelected() >= 0 && lbbrlLogicList->getSelected() < hCustomLogics->GetLogicsCount());
    if( validselect ){
...
void State::EditingWW::ApplicationStartup()
{
      if( GV->szCmdLine[0] != '\0' ){
       if( GV->szCmdLine[0] == '"' ){
        char tmp[strlen(GV->szCmdLine)-1];
         ...
         if((dp  = opendir("brush")) != NULL){
          while ((dirp = readdir(dp)) != NULL) {
           if( dirp->d_name[0] != '.' ){
            ...
            delete [] ext;
            if( !strcmp(extl, "lua") ){
             char path[7+strlen(dirp->d_name)];
             sprintf(path, "brush/%s", dirp->d_name);
             unlink(path);
             GV->Console->Printf("- Deleted ~y~%s~w~", dirp->d_name);
            }
            delete [] extl;
           }
          }
...
void State::EditingWW::SyncWorldOptionsWithParser()
{
    ...
    if( MDI->GetActiveDoc()->iWapMapBuild <= 0 ){
     ...
     sprintf(tmp, "WapMap v%s (b%d)", MDI->GetActiveDoc()->strWapMapVersion.c_str(), MDI->GetActiveDoc()->iWapMapBuild);
    }
...
void State::EditingWW::SaveWorldOptions()
{
    if( !strlen(tfwpName->getText().c_str()) ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "NoName"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }else if( strlen(tfwpName->getText().c_str()) > 64 ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "TooLongName"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }
    if( !strlen(tfwpAuthor->getText().c_str()) ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "NoAuthor"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }else if( strlen(tfwpAuthor->getText().c_str()) > 64 ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "TooLongAuthor"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }
    if( !strlen(tfwpREZ->getText().c_str()) ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "NoRez"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }else if( strlen(tfwpREZ->getText().c_str()) > 256 ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "TooLongRez"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }
    if( !strlen(tfwpTiles->getText().c_str()) ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "NoTiles"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }else if( strlen(tfwpTiles->getText().c_str()) > 128 ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "TooLongTiles"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }
    if( !strlen(tfwpPalette->getText().c_str()) ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "NoPalette"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }else if( strlen(tfwpPalette->getText().c_str()) > 128 ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "TooLongPalette"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }
    if( !strlen(tfwpExe->getText().c_str()) ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "NoExe"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }else if( strlen(tfwpExe->getText().c_str()) > 128 ){
     GV->StateMgr->Push(new State::Error(GETL(Lang_Error), GETL2S("MapProperties", "TooLongExe"), ST_ER_ICON_FATAL, ST_ER_BUT_OK, 0));
     return;
    }
    winWorld->setVisible(0);
    bool bchange = 0;
...
*/
    ...
    if( hMruList->IsValid() ){
     ...
     for(int i=0;i<hMruList->GetFilesCount();i++){
      ...
      hgeSprite * ico = (gt == WWD::Game_Claw && ibase > 0 && ibase < 15 ? GV->sprLevelsMicro16[ibase-1] : GV->sprGamesSmall[gt]);
