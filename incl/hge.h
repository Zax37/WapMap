...
public:
	...
	virtual std::vector<char*> CALL System_GetDroppedFiles() = 0;
	virtual void        CALL    System_GetDroppedFilesPosition(int * x, int * y) = 0;
	virtual void *      CALL    System_GetNotifyParam() = 0;
	virtual void        CALL    System_DoManualMainLoop() = 0;
...
public:
	...
	virtual float       CALL    Timer_GetDeltaRealtime() = 0;
	...
	virtual void        CALL    Gfx_RenderBumpedQuad(const hgeBumpQuad * quad, int lightX, int lightY) = 0;
	...
	virtual IDirect3DDevice9* CALL Gfx_GetDevice() = 0;
	virtual void        CALL    Gfx_GetClipping(int * x = NULL, int * y = NULL, int * w = NULL, int * h = NULL) = 0;
	virtual void        CALL    Gfx_FlushBuffer();

/*	virtual HSHADER		CALL	Shader_Create(const char *filename, DWORD size=0) = 0;
	virtual void		CALL	Shader_Free(HSHADER shader) = 0;
	virtual void		CALL	Gfx_SetShader(HSHADER shader) = 0;
	virtual HSHADER		CALL	Gfx_GetShader() = 0;*/

	virtual HSHADER     CALL    Shader_Create(const char * filename, DWORD size) = 0;
	virtual HSHTECH     CALL    Shader_GetTechnique(HSHADER shad, const char * name) = 0;
	virtual void        CALL    Shader_SetTechnique(HSHADER shad, HSHTECH tech) = 0;
	virtual int         CALL    Shader_Begin(HSHADER shad, int flags) = 0;
	virtual void        CALL    Shader_End(HSHADER shad) = 0;
	virtual void        CALL    Shader_BeginPass(HSHADER shad, int pass) = 0;
	virtual void        CALL    Shader_EndPass(HSHADER shad) = 0;
	virtual void        CALL    Shader_CommitChanges(HSHADER shad) = 0;

	virtual HSHPARAM    CALL    Shader_GetParam(HSHADER shad, const char * name) = 0;
	virtual void        CALL    Shader_SetValue(HSHADER shad, HSHPARAM param, void * data, int length) = 0;
	virtual void        CALL    Shader_GetValue(HSHADER shad, HSHPARAM param, void * data, int length) = 0;
	virtual void        CALL    Shader_SetBool(HSHADER shad, HSHPARAM param, bool b) = 0;
	virtual bool        CALL    Shader_GetBool(HSHADER shad, HSHPARAM param) = 0;
	virtual void        CALL    Shader_SetFloat(HSHADER shad, HSHPARAM param, float f) = 0;
	virtual float       CALL    Shader_GetFloat(HSHADER shad, HSHPARAM param) = 0;
	virtual void        CALL    Shader_SetInt(HSHADER shad, HSHPARAM param, int f) = 0;
	virtual int         CALL    Shader_GetInt(HSHADER shad, HSHPARAM param) = 0;
	virtual void        CALL    Shader_SetTexture(HSHADER shad, HSHPARAM param, HTEXTURE tex) = 0;
	virtual HTEXTURE    CALL    Shader_GetTexture(HSHADER shad, HSHPARAM param) = 0;
	virtual void        CALL    Shader_SetVector(HSHADER shad, HSHPARAM param, float x, float y, float z, float w) = 0;
	virtual void        CALL    Shader_GetVector(HSHADER shad, HSHPARAM param, float * x, float * y, float * z, float * w) = 0;
	...
	virtual HSURFACE    CALL    Target_GetSurface(HTARGET target) = 0;

	virtual void        CALL    Surface_Free(HSURFACE surf) = 0;
	virtual DWORD*      CALL    Surface_Lock(HSURFACE surf, bool bReadOnly=true, int left=0, int top=0, int width=0, int height=0) = 0;
	virtual void        CALL    Surface_Unlock(HSURFACE surf) = 0;
