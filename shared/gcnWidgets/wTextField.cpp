...
namespace SHR
{
    ...
    void TextField::renderFrame(int dx, int dy, int w, int h, unsigned char alpha, bool version)
    {
        ...
        q.v[0].x = dx+1;            q.v[0].y = dy+4;
        q.v[1].x = dx+w-1; q.v[1].y = dy+4;
        q.v[2].x = dx+w-1; q.v[2].y = dy+h-4;
        q.v[3].x = dx+1;            q.v[3].y = dy+h-4;
    ...
    void TextField::keyPressed(KeyEvent& keyEvent)
    {
        ...
        /*else if( hge->Input_GetKeyState(HGEK_CTRL) ){
         ...
         if( cb != NULL ){
          ...
          if( bNumerical ){
           for(int i=0;i<strlen(cb);i++){
            if( cb[i] < 48 && cb[i] != 45 || cb[i] > 57 || (cb[i] == 45 && (i != 0 || pastepos != 0) || !bAllowNegative) )
             bAllow = 0;
           }
          }
          if( bAllow ){
         ...
         mSelectionPosition = mText.length();
                 ...
                 key.isNumber() ||
                 key.getValue() == '-' && bNumerical && (mCaretPosition == 0 || mSelectionPosition == 0) && bAllowNegative )
        {
