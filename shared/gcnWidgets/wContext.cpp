...
namespace SHR
{
	...
	void Context::draw(Graphics* graphics)
	{
		...
		q.v[0].x = x; q.v[0].y = y + 5;
		q.v[1].x = q.v[0].x + getWidth() - 1; q.v[1].y = q.v[0].y;
		q.v[2].x = q.v[1].x; q.v[2].y = q.v[0].y + getHeight() - 10;
		...
		_ghGfxInterface->sprDropDown[1][0][1]->RenderStretch(x + 5, y + getHeight() - 5, x + getWidth() - 5, y + getHeight());
		...
		for (int i = 0; i < vElements.size(); i++) {
			if (vElements[i]->fTimer > 0.0f) {
				...
				if (i == vElements.size() - 1) {
					...
					_ghGfxInterface->sprDropDown[1][1][1]->RenderStretch(x + 5, y + getHeight() - 5, x + getWidth() - 5, y + getHeight());
			...
			if (vElements[i]->GetIcon() != NULL) {
				...
				if (vElements[i]->fTimer > 0.0f) {
					...
					vElements[i]->GetIcon()->SetColor(SETA(0xFFFFFFFF, (unsigned char)(vElements[i]->fTimer*5.0f*255.0f*gtimer)));
					vElements[i]->GetIcon()->Render(x + 4, y + iRowHeight * i + 3);
			...
			if (vElements[i]->GetIcon(1) != NULL) {
				...
				if (vElements[i]->fTimer > 0.0f) {
					...
					vElements[i]->GetIcon(1)->SetColor(SETA(0xFFFFFFFF, (unsigned char)(vElements[i]->fTimer*5.0f*255.0f*gtimer)));
					vElements[i]->GetIcon(1)->Render(x + getWidth() - 20, y + iRowHeight * i + 3);
