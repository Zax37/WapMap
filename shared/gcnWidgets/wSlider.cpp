...
namespace SHR
{
    ...
    void Slider::draw(gcn::Graphics* graphics)
    {
        ...
        if( mStyle == DEFAULT ){
         ...
         if( isEnabled() )
          sliderDrawBar(dx, dy, orient, getMarkerLength(), 2, SETA(0xFFFFFFFF, (unsigned char)(fTimer*2.5f*255.0f)));
        }else{
         int markspr = (mStyle == POINTER);
         if( mOrientation == HORIZONTAL ){
          ...
          bool markerfocus = (mx > dx+v && mx < dx+v+getMarkerLength() && my > dy && my < dy+12 && mHasMouse);
          ...
          if( fTimer > 0.0f ){
           _ghGfxInterface->sprSlider[markspr][1]->SetColor(SETA(0xFFFFFFFF, (unsigned char)(fTimer*2.5f*255.0f)));
