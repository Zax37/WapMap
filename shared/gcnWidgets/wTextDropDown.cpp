...
namespace SHR
{
    ...
    void TextDropDown::draw(Graphics* graphics)
    {
        ...
        bool buttonfoc = (isEnabled() && mHasMouse && mx > x+getWidth()-20 && mx < x+getWidth() && my > y && my < y+h);
        ...
        if( fButtonTimer > 0.0f ){
         _ghGfxInterface->sprDropDownButtonFocused->SetColor(SETA(0xFFFFFFFF, (unsigned char)(fButtonTimer*2.5f*255.0f)));
        ...
        if( bTextFocused && mSelectionPosition != -1 && mSelectionPosition != mCaretPosition ){
         ...
         graphics->drawLine(getFont()->getWidth(mText.substr(0, mCaretPosition)) - mXScroll, clipArea.height - 2, getFont()->getWidth(mText.substr(0, mCaretPosition)) - mXScroll, 1);
        }
