...
public:
    ...
    sub_streambuf()
            : _inputStream(nullptr), _startPosition(0), _currentPosition(0), _endPosition(0) {
...
protected:
    int_type underflow() override {
        ...
        if (this->gptr() >= this->egptr()) {
            ...
            _inputStream->read(_internalBuffer, std::min(static_cast<size_t>(INTERNAL_BUFFER_SIZE),
                                                         static_cast<size_t>(_endPosition - _currentPosition)));
