...
std::ios::pos_type deserialize(std::basic_istream<ELEM_TYPE, TRAITS_TYPE> &stream,
                               STRING_TYPE<ELEM_TYPE, TRAITS_TYPE, ALLOCATOR_TYPE> &out, size_t length) {
    if (length > 0) {
...
std::ios::pos_type
deserialize(std::basic_istream<ELEM_TYPE, TRAITS_TYPE> &stream, VECTOR_TYPE<VECTOR_ELEM_TYPE, ALLOCATOR_TYPE> &out,
            size_t length) {
 ...
 */
template<typename ELEM_TYPE, typename TRAITS_TYPE, typename ALLOCATOR_TYPE, template<typename, typename, typename> class STRING_TYPE>
void serialize(std::basic_ostream<ELEM_TYPE, TRAITS_TYPE> &stream,
               const STRING_TYPE<ELEM_TYPE, TRAITS_TYPE, ALLOCATOR_TYPE> &value) {
...
template<typename ELEM_TYPE, typename TRAITS_TYPE, typename VECTOR_ELEM_TYPE, typename ALLOCATOR_TYPE, template<typename, typename> class VECTOR_TYPE>
void serialize(std::basic_ostream<ELEM_TYPE, TRAITS_TYPE> &stream,
               const VECTOR_TYPE<VECTOR_ELEM_TYPE, ALLOCATOR_TYPE> &value) {
    static_assert(sizeof(ELEM_TYPE) == sizeof(VECTOR_ELEM_TYPE), "sizes must match");
