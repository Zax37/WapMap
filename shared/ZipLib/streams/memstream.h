...
public:
    basic_imemstream(ELEM_TYPE *buffer, size_t length)
            : std::basic_istream<ELEM_TYPE, TRAITS_TYPE>(&_memStreambuf), _memStreambuf(buffer, length) {
...
public:
    basic_omemstream(ELEM_TYPE *buffer, size_t length)
            : _memStreambuf(buffer, length), std::basic_ostream<ELEM_TYPE, TRAITS_TYPE>(&_memStreambuf) {

    }

    template<size_t N>
    basic_omemstream(ELEM_TYPE (&buffer)[N])
            : _memStreambuf(buffer, N), std::basic_ostream<ELEM_TYPE, TRAITS_TYPE>(&_memStreambuf) {
...
public:
    basic_iomemstream(ELEM_TYPE *buffer, size_t length)
            : _memStreambuf(buffer, length), std::basic_iostream<ELEM_TYPE, TRAITS_TYPE>(&_memStreambuf) {

    }

    template<size_t N>
    basic_iomemstream(ELEM_TYPE (&buffer)[N])
            : _memStreambuf(buffer, N), std::basic_iostream<ELEM_TYPE, TRAITS_TYPE>(&_memStreambuf) {
