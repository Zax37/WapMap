...
void SHR::cConsole::Render()
{
    ...
    if( m_bFocused || !m_bFocused && m_fAnim > -int(hge->System_GetState(HGE_SCREENHEIGHT)/3) ){
     ...
     hge->Gfx_SetClipping(0, 0, hge->System_GetState(HGE_SCREENWIDTH), hge->System_GetState(HGE_SCREENHEIGHT)/3+m_fAnim);
     if( m_sprBG != NULL ){
...
void SHR::cConsole::ListVars()
{
    ...
    char tmpstr[int(scrWidth/fntWidth)];
