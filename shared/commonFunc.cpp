...
void SHR::CopyDirR(const char * szSource, const char * szDest)
{
    DIR *dp;
    struct dirent *dirp;
    if((dp  = opendir(szSource)) == NULL) {
     return;
    }
    std::string strSourcePath = szSource;
    while ((dirp = readdir(dp)) != NULL) {
     if( dirp->d_name[0] == '.' && (dirp->d_name[1] == '\0' || dirp->d_name[1] == '.' && dirp->d_name[2] == '\0') ) continue;
     std::string strAbsEntity = strSourcePath + "/" + dirp->d_name,
                 strAbsDest = std::string(szDest) + "/" + dirp->d_name;
     struct stat st;
     stat(strAbsEntity.c_str(), &st);
